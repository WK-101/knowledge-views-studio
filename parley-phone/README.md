...
| **Circle (v3.2)** | An optional **Circle** tab (or a section at the top of Favourites) of people you want to stay close to, sorted by who's due, with **suggestions from your calls**; **log a visit, message, video call or note** and a **timeline** on each contact; **"Log this?"** after you message someone from Parley; **kind reminders**: every N days or a learned **natural rhythm**, a **weekly digest** by default, "Not now" never escalates, no streaks or red badges; birthday reminders **days ahead** with **Mark as wished**; a **People card** in Insights (reach this month, open loops, who reaches out first, year in review); a **Circle widget** that shows only counts while locked; an opt-in **"Anything to remember?"** after calls, **follow-up reminders**, **promises** (`[ ]` lines you tick off) and a **pre-call peek**; **"remember yearly"** life events; **good time to call** from your own history and their local time |
| **More in v3.2** | **Picture-in-picture** during calls (mute, hang up); **Block & decline** on the incoming screen with Undo; the call screen honours Hide screen content; **"Can't make Parley the default phone app?"** guide; **clear call history** (all, unknown, missed) with export first; failure banner with Retry; distinct answer/decline haptics; call list **grouped, every call or by day**; **"Who's in…"** a city (no location access); **label policies** (preferred SIM, rhythm, allow through Do Not Disturb); **simple mode** for an older relative, set up and shared as an encrypted file or QR; **"Where did you meet?"** when you swap cards by QR; **Markdown notes export** for Obsidian that never overwrites your edits; onboarding permissions page; one-time tips; fixed call colours; "Back up first?" before big changes and a gentle backup reminder; upright, memory-safe contact photos |
| **New in v3.3** | **Scan QR without camera permission** (one photo with your camera app, a picked screenshot, a shared image or a pasted link; decoded offline): contact cards in every common format (control flags like "starred" only if you tick them), phone, SMS, e-mail, Wi-Fi, calendar and map codes, and **19 messenger codes** (WhatsApp, Signal, Telegram, WeChat, LINE, Viber, Threema…) opened directly in the installed app; web links show the real address with look-alike warnings; **optional combined layouts**: keypad docked in Recents (folds away, keys never move) and favourites as a section or avatar strip in Contacts, both off by default with a one-tap way back; **Call** first wherever "Message on…" appears, for saved and unsaved numbers; **My card** opens its QR in one tap; **Recents at a glance**: shape-coded call badges, tinted "Call back" rows for unreturned missed calls, repeat counts with order dots, duration bars and a legend (Simple style available) |
...
| Contacts | Search (also addresses, notes, companies, websites, handles), Scan QR code, Labels, Lock now (with the app lock) | Select all, Add several numbers…, Scan QR code, Find & merge duplicates, Contacts settings (label/account/private filter chips and "Temporary (n)" stay above the list, then "My card"); multi-select ⋮ adds "Introduce myself…" and "Copy as text" |
| Keypad | Search (all contacts), Speed dial | Speed dial, SIMs & plan minutes, Scan QR code, Keypad settings |
...
**Extras (v3.2).** Contacts ⋮ and Circle ⋮ › "Who's in…" lists people linked to a city you type (address, note or where their number is from; no location permission). A label's page sets a SIM for its members, a keep-in-touch rhythm for when they join the Circle and "Allow through Do Not Disturb" (stars them, links to Android's setting). Settings › Appearance › Simple mode sets up big photo tiles, a large keypad and a question before declining, and shares the setup as an encrypted file or QR code. A contact received by encrypted QR gets "Met at … on …" and can show your own card back ("Swap"). Settings › Backup & sync › Export notes as Markdown writes one `.md` per person to a folder (one way).

**Scan QR (v3.3).** Contacts header (and ⋮), Keypad ⋮, My card › QR code › "Scan theirs", Settings › Contacts › Scan QR code, the launcher shortcut "Scan QR" and a Quick Settings tile open the scan screen. Parley has no camera permission: "Take a photo" asks the phone's camera app for one picture (kept in Parley's cache and deleted once read), "Pick an image" uses the system photo picker, a picture can be shared to Parley, and "Paste" reads the clipboard only when tapped. Codes are read on the phone with ZXing (several codes in one picture, inverted and turned codes). The sheet says what the code holds and acts only on a tap: contacts (vCard 2.1–4.0, MECARD, BIZCARD; add, add privately, add as temporary, add to an existing contact, import several), numbers (service codes with * or # only go on the keypad), SMS, email, places, Wi-Fi (Android 11+ asks to save the network itself; otherwise copy the password and open Wi-Fi settings), calendar events, chat links of about 20 messengers (opened in the app itself; if it's missing: copy, browser or store page) and web addresses (the owning domain shown large, with warnings for look-alike letters, well-known names in front of another domain, shorteners, `user@host` tricks and bare IP addresses).
