...
| Recents | Search, Call insights | Export…, Messaged numbers, Call history settings (filter chips and saved filters stay above the list) |
| Contacts | Search, Labels, Lock now (with the app lock) | Select all, Add several numbers…, Find & merge duplicates, Contacts settings (label/account/private filter chips and "Temporary (n)" stay above the list); multi-select ⋮ adds "Introduce myself…" |
...
| Messaging | Quick reply messages, My details for messages · Messaged numbers, Forget messaged numbers after |
