...
fun DryRunScreen(vm: AppViewModel, back: () -> Unit) {
    ...
    Scaffold(topBar = {
        TopAppBar(title = { Text("What would today's rules do?") }, navigationIcon = { IconButton(back) { Icon(Icons.AutoMirrored.Rounded.ArrowBack, "Back") } })
    }) { p ->
        LazyColumn(Modifier.padding(p)) {
            item {
                Row(Modifier.padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    listOf(1, 7, 30).forEach { d -> FilterChip(days == d, { days = d }, label = { Text(if (d == 1) "Today" else "$d days") }) }
                }
                Card(Modifier.fillMaxWidth().padding(16.dp)) {
                    Column(Modifier.padding(16.dp)) {
                        Text(
                            when {
                                loading -> "Replaying your calls…"
                                result == null -> "Couldn't read your call history."
                                else -> "Current rules " + result!!.current.summary().replaceFirstChar { it.lowercase() } + "."
                            },
                            style = MaterialTheme.typography.titleMedium,
                        )
                        Text("Only a replay: nothing is blocked, logged, counted or sent. Hidden numbers and SIM rules are replayed as the phone app would see them.", style = MaterialTheme.typography.bodySmall)
                    }
                }
            }
            ...
            items(rows, key = { "${it.call.time}|${it.call.number}" }) { r ->
                ...
                Column {
                    ListItem(
                        ...
                        leadingContent = { Icon(if (r.result.blocked) Icons.Rounded.Block else Icons.Rounded.Call, if (r.result.blocked) "Would be blocked" else "Would ring") },
                        headlineContent = { Text(if (r.call.hidden) "Private number" else Format.number(r.call.number, vm.countryIso)) },
                        supportingContent = { Text(Format.fullDate(context, r.call.time) + " · " + (r.result.verdict?.text ?: if (r.result.blocked) "Would be blocked" else "Would ring")) },
                    )
