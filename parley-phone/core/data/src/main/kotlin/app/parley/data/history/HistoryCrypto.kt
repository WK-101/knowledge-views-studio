...
internal class HistoryCrypto(context: Context) {
    ...
    /** Forgets the key (after [KeyLostException]); the next use creates a new one. */
    ...
    private fun unwrap(blob: ByteArray): ByteArray {
        ...
        val key = keyStore().getKey(ALIAS, null) as? SecretKey ?: throw IllegalStateException("Wrapping key missing")
        val c = Cipher.getInstance("AES/GCM/NoPadding")
