...
export default class KnowledgeViewsStudioPlugin extends Plugin {
  ...
  override async onload(): Promise<void> {
    ...
    const applyImageVars = (): void => {
      ...
      // its real colour — Zotero's, or a custom override — and updates the moment the palette changes. The
      // hex feeds the inline marks; the RGB triple feeds the imported-annotation callouts' --callout-color.
      for (const c of effectivePalette(s.paletteOverride)) {
        document.body.style.setProperty(`--kvs-mark-${c.name}`, c.hex);
        document.body.style.setProperty(`--kvs-mark-rgb-${c.name}`, c.rgb.join(", "));
      }
    };
    applyImageVars();
