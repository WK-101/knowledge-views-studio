...
export class DashboardView extends TextFileView {
  ...
  /** Resolve a kit semantic field to a column (explicit field map first, then type/name) so lookups
   *  survive column renames. */
  private fieldCol(cols: readonly { name: string; type: string }[], field: AcademicField): { name: string; type: string } | undefined {
    return resolveFieldColumn(cols, field, (this.renderedProfile() ?? this.currentProfile())?.fieldMap);
  }
  ...
  /** Fetch metadata for a library row's DOI and fill its empty authors/title/year/venue cells. */
  /** Look up DOI metadata and return column→value for the card's fetch button. */
  private async fetchDoiValues(doi: string): Promise<Record<string, string> | null> {
    if (!this.deps.store.getSettings().researchLookupEnabled) {
    ...
    const cols = (this.renderedProfile() ?? this.currentProfile())?.columns ?? [];
    return this.doiRowValues(cols, res.meta, doi);
  }

  /** Fetch Crossref metadata for a DOI through Obsidian's network layer (metadata or null). */
  private fetchDoi(doi: string): Promise<DoiMetadata | null> {
    return fetchDoiMetadata(doi, (url) => this.lookupFetch(url));
  }

  /** Diagnosed DOI lookup — returns the metadata or a human-readable failure reason. */
  private lookupDoi(doi: string): Promise<DoiLookupResult> {
    return fetchDoiMetadataResult(doi, (url) => this.lookupFetch(url));
  }

  /** Shared network fetch for lookups — appends the configured contact email (polite pool). */
  private async lookupFetch(url: string): Promise<{ status: number; json?: unknown; text?: string }> {
    const email = this.deps.store.getSettings().researchEmail.trim();
  ...
  /** Edits that fill a row's EMPTY authors/title/year/venue cells from DOI metadata. */
  ...
  private async fillFromDoi(row: Row, profile: Profile): Promise<void> {
    if (!this.deps.store.getSettings().researchLookupEnabled) {
      new Notice("Enable research lookups in Settings → Academic Research kit.");
      return;
    }
    ...
    await this.applyRowEdits(row.provenance.filePath, edits, `Fill ${edits.length} field(s) from DOI`);
    new Notice(`Filled ${edits.length} field(s) from DOI.`);
  }

  /** Fill every library row with a DOI but missing metadata, throttled to be polite to Crossref. */
  async bulkFillFromDoi(): Promise<void> {
    ...
    const delay = this.deps.store.getSettings().researchRequestDelayMs;
    ...
    if (allEdits.length > 0) {
      ...
      const res = await this.deps.writer.editCells(allEdits);
      if (res.applied > 0) {
        this.deps.undo.push({
          label: `Fill ${res.applied} field(s) from DOI`,
          undo: async () => {
            await this.deps.writer.restore(snapshot);
            for (const p of paths) this.deps.dataService.invalidate(p);
  ...
  /** Fetch a list of DOIs and append them as new rows to this view's library note. */
  async captureByDoi(dois: readonly string[]): Promise<void> {
    ...
    const cols = (this.renderedProfile() ?? profile).columns;
    const reference = this.lastRows[0]?.provenance;
    if (!reference) {
    ...
    for (let i = 0; i < dois.length; i++) {
      ...
      try {
        const res = await this.deps.writer.appendRow(reference, this.doiRowValues(cols, meta, doi));
        if (res.ok) added++;
        else failed++;
      } catch {
        failed++;
  ...
  /** Scan the library for rows sharing a DOI and open the resolver. */
  async findDuplicateDois(): Promise<void> {
    ...
    const result = await this.deps.dataService.query({ ...profile, pageSize: null }, {});
    ...
    new DedupModal(this.app, groups, titleCol?.name ?? null, keyCol?.name ?? null, (res) => void this.resolveDuplicates(res)).open();
  }
  ...
  private async resolveDuplicates(res: DedupResolution): Promise<void> {
    ...
    if (res.mergeEdits.length > 0) await this.deps.writer.editCells(res.mergeEdits.map((e) => ({ provenance: e.provenance, column: e.column, value: e.value })));
    const del = await this.deps.writer.deleteRows(res.remove.map((r) => r.provenance));
    if (!del.ok) {
      ...
      openRowDetail(
        {
          ...
          ...(base.academicKit ? { onFetchDoiValues: (doi: string) => this.fetchDoiValues(doi) } : {}),
          ...(base.academicKit ? { onFindCitations: (doi: string) => this.findCitationsFor(doi) } : {}),
        },
        newRow,
      );
    }
  ...
  private async citationLibraryMap(doiCol: { name: string }, citeCol: { name: string }, force: boolean): Promise<Map<string, string>> {
    ...
    const profile = this.renderedProfile() ?? this.currentProfile();
    const result = profile ? await this.deps.dataService.query({ ...profile, pageSize: null }, {}) : { rows: [] as Row[] };
  ...
  /** Per-paper citation lookup for the card's button: returns column→value (Cites + optional checked
   *  date). Overwrites Cites so it always reflects the latest lookup. */
  private async findCitationsFor(doi: string): Promise<Record<string, string> | null> {
    if (!this.deps.store.getSettings().researchLookupEnabled) {
      new Notice("Enable research lookups in Settings → Academic Research kit.");
      return null;
    }
  ...
  /** Import references into this library, skipping any already present (matched by DOI or cite key). */
  async importReferences(refs: readonly ImportedRef[]): Promise<void> {
    ...
    for (let i = 0; i < refs.length; i++) {
      ...
      try {
        const res = await this.deps.writer.appendRow(reference, this.refToRowValues(cols, ref));
        if (res.ok) {
          added++;
  ...
  /** Filesystem-safe shard name from a field value. */
  ...
  /** Open the shard picker for the current library view. */
  async openShardModal(): Promise<void> {
    ...
    new ShardModal(this.app, fields, (field) => void this.shardBy(field, profile)).open();
  }

  /** Split the library's rows into one note per value of `field`, then remove the originals.
   *  Undoable: restores the source files and deletes the shard notes. */
  private async shardBy(field: string, profile: Profile): Promise<void> {
    ...
    const snapshot = await this.deps.writer.snapshot(sourcePaths);
    ...
    try {
      for (const [bucket, rows] of buckets) {
        ...
        for (let n = 2; this.app.vault.getAbstractFileByPath(path) || sourcePaths.includes(path); n++) {
          path = `${folder}/${bucket} (${n}).md`;
        }
        await this.app.vault.create(path, this.shardNote(bucket, field, headers, rows));
      ...
      for (const [, provs] of byFile) await this.deps.writer.deleteRows(provs);
      ...
      this.deps.undo.push({
        ...
        undo: async () => {
          await this.deps.writer.restore(snapshot);
          await removeShards();
          for (const p of sourcePaths) this.deps.dataService.invalidate(p);
          void this.renderActive();
        },
      });
      for (const p of [...sourcePaths, ...createdShards]) this.deps.dataService.invalidate(p);
      void this.renderActive();
      notice.hide();
  ...
  /** Build a shard note: a heading + a Markdown table of the bucket's rows. */
  ...
  private async promoteToNote(row: Row, profile: Profile): Promise<void> {
    ...
    const doi = val("doi");
    const tagCol = this.fieldCol(cols, "tags");
  ...
  private async renderActive(): Promise<void> {
    ...
    await renderProfile({
      ...
      ...(profile.academicKit ? { onFetchDoiValues: (doi: string) => this.fetchDoiValues(doi) } : {}),
      ...(profile.academicKit ? { onFindCitations: (doi: string) => this.findCitationsFor(doi) } : {}),
      ...(profile.academicKit ? { onCite: (citeKey: string) => this.insertCitation(citeKey) } : {}),
      ...(profile.academicKit
        ? {
            onFetchDoi: (row: Row) => void this.fillFromDoi(row, profile),
