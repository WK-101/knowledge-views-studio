...
// FTS_BODY_CHARS (the full-text index ceiling) now lives in ExtractionSupport.kt, shared with
...
class FeedRepository @Inject constructor(
    ...
    private val articleExtraction: ArticleExtractionService,
    ...
    private suspend fun insertParsed(
        ...
        if (isNew && wantsFullText) {
            p.link?.takeIf { it.isNotBlank() }?.let { coRunCatching { articleExtraction.extractInto(itemId, it) } }
        }
    ...
    suspend fun saveUrl(rawUrl: String): Result<String> {
        ...
        articleExtraction.extractInto(itemId, url)
    ...
    suspend fun saveArchivedUrl(source: SourceEntity, rawUrl: String, lastmod: Long?, mirror: Boolean = false): Boolean {
        ...
        coRunCatching { articleExtraction.extractInto(itemId, url) }
    ...
    /**
     * On-demand article extraction is owned by [ArticleExtractionService]. FeedRepository keeps these
     * thin delegates so every existing call site (the reader, share, save flows) is unchanged, while
     * the ~240 lines of Readability / JS-render / archive-recovery logic live in one focused,
     * independently-testable place.
     */
    suspend fun extractFull(itemId: String) = articleExtraction.extractFull(itemId)

    suspend fun extractWithJs(itemId: String): Boolean = articleExtraction.extractWithJs(itemId)
    ...
    /** Public-archive paywall recovery — see [ArticleExtractionService.saveFromArchive]. */
    suspend fun saveFromArchive(itemId: String): Boolean = articleExtraction.saveFromArchive(itemId)
    ...
    suspend fun saveOffline(itemId: String): Result<Int> {
        ...
        if (item.extractStatus != ExtractStatus.OK.raw || item.blobPath.isNullOrBlank()) {
            coRunCatching { articleExtraction.extractInto(itemId, item.url) }
        }
    ...
    suspend fun healLink(itemId: String): Boolean {
        ...
        coRunCatching { articleExtraction.extractInto(itemId, rawSnapshot) }
    ...
    // hostOf(url) now lives in ExtractionSupport.kt (shared with ArticleExtractionService).
