...
import com.wkhan.hexis.domain.NoteEditing
import com.wkhan.hexis.ui.components.borderlessFieldColors
...
fun NoteBodyEditor(
    ...
    type: com.wkhan.hexis.domain.NoteAppearance.NoteType = com.wkhan.hexis.domain.NoteAppearance.NoteType(),
    ...
    Column(modifier.fillMaxSize()) {
        ...
        val transform: androidx.compose.ui.text.input.VisualTransformation = remember(liveStyle, cs) {
            if (liveStyle) com.wkhan.hexis.ui.components.MarkdownVisualTransformation(
 ...
 * [com.wkhan.hexis.domain.MarkdownTable] serializes it. No more hand-aligning pipes.
 */
@Composable
fun TableEditorDialog(initial: String? = null, onInsert: (String) -> Unit, onDismiss: () -> Unit) {
    val start = remember { initial?.let { com.wkhan.hexis.domain.MarkdownTable.parse(it) } ?: com.wkhan.hexis.domain.MarkdownTable.empty(2, 2) }
    ...
    fun build(): String {
        val t = com.wkhan.hexis.domain.MarkdownTable.Table(headers.toList(), rows.map { it.cells.toList() })
        return com.wkhan.hexis.domain.MarkdownTable.serialize(t)
    }
    AlertDialog(
 ...
 * Pure logic in [com.wkhan.hexis.domain.MarkdownSections].
 */
@Composable
fun SectionReorderDialog(body: String, onApply: (String) -> Unit, onDismiss: () -> Unit) {
    var working by remember { mutableStateOf(body) }
    val sections = remember(working) { com.wkhan.hexis.domain.MarkdownSections.sections(working) }
    AlertDialog(
        onDismissRequest = onDismiss,
                ...
                LazyColumn(Modifier.heightIn(max = 380.dp)) {
                    items(sections.size) { i ->
                        ...
                        Row(Modifier.fillMaxWidth().padding(vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Text("↑", Modifier.clickable { if (i > 0) working = com.wkhan.hexis.domain.MarkdownSections.move(working, i, i - 1) }.padding(8.dp),
                                color = if (i > 0) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = .4f))
                            Text("↓", Modifier.clickable { if (i < sections.lastIndex) working = com.wkhan.hexis.domain.MarkdownSections.move(working, i, i + 1) }.padding(8.dp),
...
@Composable
fun NoteWrappedDialog(stats: com.wkhan.hexis.domain.NoteWrapped.Stats, onDismiss: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
...
fun NoteTemplateDialog(
    custom: List<com.wkhan.hexis.domain.NoteTemplates.Template>,
    onPick: (com.wkhan.hexis.domain.NoteTemplates.Template) -> Unit,
        ...
        text = {
            if (creating) {
                ...
                LazyColumn(Modifier.heightIn(max = 400.dp)) {
                    ...
                    items(com.wkhan.hexis.domain.NoteTemplates.ALL, key = { it.id }) { t ->
...
@Composable
fun RelatedNotesDialog(hits: List<com.wkhan.hexis.domain.NoteRelated.Hit>, onOpen: (String) -> Unit, onDismiss: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
        confirmButton = { TextButton(onClick = onDismiss) { Text("Done") } },
 ...
 * key:value pairs; on apply the pure [com.wkhan.hexis.domain.NoteProperties] rewrites the leading
...
fun NotePropertiesDialog(body: String, onApply: (String) -> Unit, onDismiss: () -> Unit) {
    val keys = remember { mutableStateListOf<String>().apply { addAll(com.wkhan.hexis.domain.NoteProperties.parse(body).keys) } }
    val vals = remember { mutableStateListOf<String>().apply { addAll(com.wkhan.hexis.domain.NoteProperties.parse(body).values) } }
    fun build(): String {
        val m = LinkedHashMap<String, String>()
        for (i in keys.indices) { val k = keys[i].trim(); if (k.isNotEmpty()) m[k] = vals.getOrElse(i) { "" } }
        return com.wkhan.hexis.domain.NoteProperties.withProperties(body, m)
    }
    AlertDialog(
        onDismissRequest = onDismiss,
...
fun NoteAboutDialog(note: NoteEntity, onDismiss: () -> Unit) {
    ...
    val outLinks = com.wkhan.hexis.domain.NoteGrammar.WIKI_LINK.findAll(note.body).count()
    val tagCount = com.wkhan.hexis.domain.NoteGrammar.TAG.findAll(note.body).count()
 ...
 *  Markdown lint nits. Pure logic lives in [com.wkhan.hexis.domain.NoteOutline] / NoteLint. */
@Composable
fun NoteOutlineDialog(body: String, onDismiss: () -> Unit) {
    val headings = remember(body) { com.wkhan.hexis.domain.NoteOutline.outline(body) }
    val stats = remember(body) { com.wkhan.hexis.domain.NoteOutline.stats(body) }
    val issues = remember(body) { com.wkhan.hexis.domain.NoteLint.lint(body) }
    AlertDialog(
        onDismissRequest = onDismiss,
        confirmButton = { TextButton(onClick = onDismiss) { Text("Done") } },
            ...
            AlertDialog(
                ...
                text = {
                    Column(Modifier.verticalScroll(rememberScrollState())) {
                        ...
                        LazyRow(horizontalArrangement = Arrangement.spacedBy(6.dp), modifier = Modifier.padding(vertical = 4.dp)) {
                            items(com.wkhan.hexis.domain.recurrence.Recurrence.PRESETS, key = { it.second }) { (rule, label) ->
