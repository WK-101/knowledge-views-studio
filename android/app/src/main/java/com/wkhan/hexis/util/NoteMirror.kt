...
/**
 * Wave N — the living two-way `.md` mirror. Turns Hexis's one-shot folder export/import into a working
 ...
 * doesn't look like a change. The baseline map lives in a folder-local `.hexis/mirror.json`, so the
...
object NoteMirror {
    ...
    /** One note's last-synced state, persisted in `.hexis/mirror.json`. */
