...
import com.wkhan.hexis.domain.Modules
...
import com.wkhan.hexis.ui.components.AppDrawer
import com.wkhan.hexis.ui.components.ConfirmDialog
import com.wkhan.hexis.domain.OmegaCommand
import com.wkhan.hexis.ui.screens.CalendarScreen
import com.wkhan.hexis.ui.screens.CommandPaletteDialog
import com.wkhan.hexis.ui.screens.MatrixScreen
import com.wkhan.hexis.ui.screens.QuickAddSheet
import com.wkhan.hexis.ui.screens.RecapScreen
import com.wkhan.hexis.ui.screens.SearchScreen
import com.wkhan.hexis.ui.screens.SettingsScreen
import com.wkhan.hexis.ui.screens.TaskDetailScreen
import com.wkhan.hexis.ui.screens.TasksScreen
...
/**
 * R109 (NavHost round · stage 2) — the argument-free full-screen overlays, as one back-stack destination set
 * (replacing ~13 independent `show*` booleans). Argument-carrying overlays (Countdowns / DayReview / Recap /
 * Journal) and the VM-held habit/life screens are not here yet; they fold into the same stack in a later stage.
 */
...
private sealed interface OverlayArg {
    ...
    data class Journal(val period: com.wkhan.hexis.domain.PeriodRange, val anchor: Long) : OverlayArg
            ...
            restore = { s ->
                if (s.isEmpty()) null else {
                    ...
                    when (p[0]) {
                        ...
                        "J" -> Journal(com.wkhan.hexis.domain.PeriodRange.valueOf(p[1]), p[2].toLong())
                        else -> null
                    }
                }
            },
...
fun AppRoot(
    ...
    val vm: AppViewModel = viewModel(
        factory = AppViewModelFactory(
            androidx.compose.ui.platform.LocalContext.current.applicationContext as com.wkhan.hexis.App,
        ),
    )
    ...
    AppTheme(themeMode = settings.themeMode, dynamicColor = settings.dynamicColor, accentArgb = settings.accentArgb) {
      ...
      androidx.compose.runtime.CompositionLocalProvider(
        com.wkhan.hexis.ui.components.LocalColorPickerHost provides com.wkhan.hexis.ui.components.ColorPickerHost(colorRecents, vm::rememberRecentColor)
      ) {
      AppLockGate(enabled = settings.appLockEnabled, wipeAfter = settings.appLockWipeAfter, onWipe = { vm.panicWipe() }) {
        ...
        var manageTag by remember { mutableStateOf<com.wkhan.hexis.data.entity.TagEntity?>(null) }
        var moveTag by remember { mutableStateOf<com.wkhan.hexis.data.entity.TagEntity?>(null) }
        var newCtx by remember { mutableStateOf<NewTagReq?>(null) }
        var manageCtx by remember { mutableStateOf<com.wkhan.hexis.data.entity.ContextEntity?>(null) }
        var moveCtx by remember { mutableStateOf<com.wkhan.hexis.data.entity.ContextEntity?>(null) }
        var newWs by remember { mutableStateOf(false) }
        var manageWs by remember { mutableStateOf<com.wkhan.hexis.data.entity.WorkspaceEntity?>(null) }
        var filterEdit by remember { mutableStateOf<com.wkhan.hexis.data.entity.FilterEntity?>(null) }
        // R109 (NavHost round · stage 2) — the argument-free full-screen overlays are ONE back-stack instead of
        // ~13 independent `show*` booleans. The top of the stack is the overlay on screen; opening pushes, each
        // screen's own Back/close pops. Momentum→Goals stacks naturally (push Goals over Momentum; Back returns
        // to Momentum). rememberSaveable so an open overlay — and the Momentum→Goals depth — survive config-change
        // and process death, exactly as the old saveable flags did.
        val overlayStack = rememberSaveable(
            saver = listSaver(save = { it.map(Overlay::name) }, restore = { it.map(Overlay::valueOf).toMutableStateList() }),
        ) { mutableStateListOf<Overlay>() }
        fun openOverlay(o: Overlay) { if (overlayStack.lastOrNull() != o) overlayStack.add(o) }
        fun closeOverlay() { if (overlayStack.isNotEmpty()) overlayStack.removeAt(overlayStack.lastIndex) }
        ...
        val currentContainerArchived = remember(currentView, lists, folders) {
            val (hiddenLists, hiddenFolders) = com.wkhan.hexis.domain.view.ListPipeline.hiddenContainers(lists, folders)
        ...
        LaunchedEffect(settings.reminderSound) {
            com.wkhan.hexis.reminders.Notifications.reminderSoundSpec = settings.reminderSound
            com.wkhan.hexis.reminders.Notifications.ensureChannel(context)
        }
        ...
        LaunchedEffect(launchAction.value) {
            ...
            when {
                a == com.wkhan.hexis.MainActivity.ACTION_QUICK_ADD -> { openQuickAdd(null); launchAction.value = null }
                a != null && a.startsWith(com.wkhan.hexis.MainActivity.ACTION_QUICK_ADD_TEXT) -> {
                    quickAddText = a.removePrefix(com.wkhan.hexis.MainActivity.ACTION_QUICK_ADD_TEXT)
                ...
                a == "open_habit_add" -> { tab = Tab.HABITS; vm.habitEditor.value = com.wkhan.hexis.ui.HabitEditRequest(); launchAction.value = null }
                ...
                a != null && a.startsWith(com.wkhan.hexis.MainActivity.ACTION_TRACK_NAME) -> {
                    val nm = a.removePrefix(com.wkhan.hexis.MainActivity.ACTION_TRACK_NAME)
                    vm.timeVm.startTimeTrackingByName(nm); openOverlay(Overlay.TIME_TRACKING); launchAction.value = null
                }
                a != null && a.startsWith(com.wkhan.hexis.MainActivity.ACTION_RUN_ROUTINE) -> {
                    val nm = a.removePrefix(com.wkhan.hexis.MainActivity.ACTION_RUN_ROUTINE)
                    vm.runRoutineByName(nm); openOverlay(Overlay.TIME_TRACKING); launchAction.value = null
                }
                // Routine reminder tap → open the Routines screen with the runner already started for it.
                a != null && a.startsWith("open_routine_run:") -> {
                    vm.requestRoutineRun(a.removePrefix("open_routine_run:")); openOverlay(Overlay.ROUTINES); launchAction.value = null
                }
        ...
        val title = when (tab) {
            Tab.TASKS -> when (val v = currentView) {
                is ViewRef.Smart -> com.wkhan.hexis.domain.smartTitle(settings, v.kind)
        ...
        ModalNavigationDrawer(
            ...
            drawerContent = {
                AppDrawer(
                    ...
                    onEditFilter = { f -> filterEdit = f ?: com.wkhan.hexis.data.entity.FilterEntity(id = java.util.UUID.randomUUID().toString(), name = "New filter", workspaceId = settings.activeWorkspaceId) },
                    onOpenStats = { openOverlay(Overlay.STATS); scope.launch { drawerState.close() } },
                    onOpenReview = { openOverlay(Overlay.REVIEW); scope.launch { drawerState.close() } },
                    ...
                    onOpenRecap = { val t = java.time.LocalDate.now(); val ws = com.wkhan.hexis.domain.weekStartOf(t, settings.weekStart); argOverlay = OverlayArg.Recap(ws.toEpochDay(), t.toEpochDay(), "This week"); scope.launch { drawerState.close() } },
            ...
            Scaffold(
                ...
                topBar = {
                    ...
                    if (tab == Tab.CALENDAR) {
                        ...
                        com.wkhan.hexis.ui.screens.CalHeader(
                            label = com.wkhan.hexis.ui.screens.calLabel(calMode, calAnchor, firstDow),
                            anchor = calAnchor, showNav = calMode != "list",
                            onPrev = { calAnchor = com.wkhan.hexis.ui.screens.calStep(calMode, calAnchor, -1) },
                            onNext = { calAnchor = com.wkhan.hexis.ui.screens.calStep(calMode, calAnchor, 1) },
                        ...
                        com.wkhan.hexis.ui.screens.HabitsHeader(vm, onOpenDrawer = { scope.launch { drawerState.open() } })
                    } else TopAppBar(
                        ...
                        actions = {
                            ...
                            if (tab == Tab.TASKS) IconButton(onClick = { openOverlay(Overlay.PLAN) }) { Icon(Icons.Filled.Bolt, "Plan your day") }
                            ...
                            when (tab) {
                                Tab.TASKS -> {
                                    ...
                                    DropdownMenu(expanded = menu, onDismissRequest = { menu = false }) {
                                        ...
                                        val doneView = (curView as? com.wkhan.hexis.domain.view.ViewRef.Smart)?.kind.let {
                                            it == com.wkhan.hexis.domain.view.SmartKind.COMPLETED || it == com.wkhan.hexis.domain.view.SmartKind.WONT_DO
                                        }
                                ...
                                Tab.TIME -> IconButton(onClick = { openOverlay(Overlay.TIME_STATS) }) { Icon(Icons.Filled.BarChart, "Time stats") }
                        ...
                        Column {
                            ...
                            CompactBottomBar(
                                ...
                                onLongPressPrimary = {
                                    ...
                                    com.wkhan.hexis.domain.view.ViewTabs.viewOf(ref)?.let { vm.select(it); tab = Tab.TASKS }
                                },
                            )
                        }
                    }
                        ...
                        Box {
                            ...
                            DropdownMenu(expanded = notesFabMenu, onDismissRequest = { notesFabMenu = false }) {
                                ...
                                DropdownMenuItem(text = { Text("This week's note") }, leadingIcon = { Text("📆") },
                                    onClick = { notesFabMenu = false; vm.openPeriodicNote(com.wkhan.hexis.domain.PeriodRange.WEEK, notesToday) { id -> editingNote = id } })
                                DropdownMenuItem(text = { Text("This month's note") }, leadingIcon = { Text("🗓️") },
                                    onClick = { notesFabMenu = false; vm.openPeriodicNote(com.wkhan.hexis.domain.PeriodRange.MONTH, notesToday) { id -> editingNote = id } })
                            }
                        }
                        ...
                        when (t) {
                            Tab.TASKS -> androidx.compose.foundation.layout.Column(Modifier.fillMaxSize()) {
                                ...
                                Box(Modifier.weight(1f)) {
                                    ...
                                    if (boardMode) com.wkhan.hexis.ui.screens.KanbanScreen(vm, ::openTask) else TasksScreen(vm, ::openTask, onOpenOccasion = openOccasion, onOpenRoutineRun = { vm.requestRoutineRun(it); openOverlay(Overlay.ROUTINES) })
                                }
                            }
          ...
          Tab.NOTES -> com.wkhan.hexis.ui.screens.NotesScreen(vm, onOpenNote = ::openNote, query = notesQuery, onQueryChange = { notesQuery = it }, searchOpen = notesSearchOpen, onOpenGraph = { openOverlay(Overlay.NOTES_GRAPH) }, onOpenGarden = { openOverlay(Overlay.NOTES_GARDEN) }, onOpenRecall = { openOverlay(Overlay.RECALL) }, onOpenJournal = { argOverlay = OverlayArg.Journal(com.wkhan.hexis.domain.PeriodRange.DAY, java.time.LocalDate.now().toEpochDay()) })
                            ...
                            Tab.TIMELINE -> com.wkhan.hexis.ui.screens.TimelineScreen(vm, ::openTask, selectedLists = timelineLists, showDone = timelineShowDone)
                            Tab.MATRIX -> MatrixScreen(vm, ::openTask, matrixSettings, { matrixSettings = false })
                            Tab.HABITS -> com.wkhan.hexis.ui.screens.HabitsScreen(vm, onFocusHabit = { hid -> vm.pendingFocusHabitId.value = hid; timeFocus = true; tab = Tab.TIME })
                            Tab.TIME, Tab.FOCUS -> Column(Modifier.fillMaxSize()) {
                                ...
                                if (timeFocus) com.wkhan.hexis.ui.screens.FocusScreen(vm, onOpenStats = { openOverlay(Overlay.TIME_STATS) })
                                else com.wkhan.hexis.ui.screens.TimeTrackingScreen(vm, onBack = {}, embedded = true)
                            }
                        }
                    }
                }
        ...
        editingNote?.let { id -> com.wkhan.hexis.ui.screens.NoteEditorScreen(vm, id,
        ...
        habitDetail?.let { hid ->
            com.wkhan.hexis.ui.screens.HabitDetailScreen(vm, hid,
        ...
        habitEdit?.let { req ->
            com.wkhan.hexis.ui.screens.HabitEditorScreen(vm, req.habit, onClose = { vm.habitEditor.value = null })
        }
        val habitTrends by vm.habitTrendsOpen.collectAsStateWithLifecycle()
        if (habitTrends) com.wkhan.hexis.ui.screens.HabitTrendsScreen(vm, onBack = { vm.habitTrendsOpen.value = false })
        val habitArchive by vm.habitArchiveOpen.collectAsStateWithLifecycle()
        if (habitArchive) com.wkhan.hexis.ui.screens.HabitArchiveScreen(vm, onClose = { vm.habitArchiveOpen.value = false })
        ...
        lifeRoute?.let { route ->
            com.wkhan.hexis.ui.screens.LifeSystemsScreen(vm, route,
        ...
        // Momentum → Goals is the one overlay→overlay edge: it pushes Goals, so Back returns to Momentum.
        when (overlayStack.lastOrNull()) {
            Overlay.STATS -> com.wkhan.hexis.ui.screens.StatisticsScreen(vm, onBack = { closeOverlay() })
            Overlay.ATTACHMENTS -> com.wkhan.hexis.ui.screens.AttachmentsScreen(vm, onOpenTask = { closeOverlay(); openTask(it) }, onBack = { closeOverlay() })
            Overlay.DONE -> com.wkhan.hexis.ui.screens.DoneScreen(vm, onOpenTask = { closeOverlay(); openTask(it) }, onBack = { closeOverlay() })
            Overlay.PLAN -> com.wkhan.hexis.ui.screens.PlanYourDayScreen(vm, onOpenTask = { closeOverlay(); openTask(it) }, onBack = { closeOverlay() })
            Overlay.REVIEW -> com.wkhan.hexis.ui.screens.ReviewScreen(vm, onOpenTask = { closeOverlay(); openTask(it) }, onBack = { closeOverlay() })
            Overlay.MOMENTUM -> com.wkhan.hexis.ui.screens.MomentumScreen(vm, onBack = { closeOverlay() }, onOpenGoals = { openOverlay(Overlay.GOALS) })
            Overlay.ROUTINES -> com.wkhan.hexis.ui.screens.RoutinesScreen(vm, onBack = { closeOverlay() })
            Overlay.GOALS -> com.wkhan.hexis.ui.screens.GoalsScreen(vm, onBack = { closeOverlay() }, onOpenNote = { nid -> closeOverlay(); editingNote = nid })
            Overlay.TIME_TRACKING -> com.wkhan.hexis.ui.screens.TimeTrackingScreen(vm, onBack = { closeOverlay() })
            Overlay.TIME_STATS -> com.wkhan.hexis.ui.screens.TimeStatsScreen(vm, onBack = { closeOverlay() })
            Overlay.NOTES_GRAPH -> com.wkhan.hexis.ui.screens.NoteGraphScreen(vm, onOpenNote = { closeOverlay(); openNote(it) }, onClose = { closeOverlay() })
            Overlay.NOTES_GARDEN -> com.wkhan.hexis.ui.screens.NoteGardenScreen(vm, onOpenNote = { closeOverlay(); openNote(it) }, onClose = { closeOverlay() })
            Overlay.RECALL -> com.wkhan.hexis.ui.screens.RecallScreen(vm, onClose = { closeOverlay() })
            else -> {}
        }
        ...
        when (val a = argOverlay) {
            is OverlayArg.Occasions -> com.wkhan.hexis.ui.screens.CountdownScreen(vm, onBack = { argOverlay = null }, initialOpenId = a.openId)
            is OverlayArg.DayReview -> com.wkhan.hexis.ui.screens.DayReviewScreen(vm, a.day, startInClose = a.startClose, startInWeekly = a.startWeekly, onOpenTask = { argOverlay = null; openTask(it) }, onOpenNote = { editingNote = it }, onBack = {
                argOverlay = null
            ...
            is OverlayArg.Journal -> com.wkhan.hexis.ui.screens.PeriodicNotesScreen(
                ...
                onOpenReview = { rp, ra ->
                    ...
                    argOverlay = when (rp) {
                        com.wkhan.hexis.domain.PeriodRange.DAY -> OverlayArg.DayReview(w.startDay, startClose = false, startWeekly = false)
                        com.wkhan.hexis.domain.PeriodRange.WEEK -> OverlayArg.DayReview(w.startDay, startClose = false, startWeekly = true)
                        else -> OverlayArg.Recap(w.startDay, w.endDay, com.wkhan.hexis.domain.PeriodicNotes.titleFor(rp, w.startDay))
                    }
                },
        ...
        if (showPalette) CommandPaletteDialog(vm, onDismiss = { showPalette = false }) { cmd ->
            ...
            when (cmd) {
                ...
                is OmegaCommand.Command.Act -> when (cmd.action) {
                    OmegaCommand.Action.PLAN -> openOverlay(Overlay.PLAN)
                    OmegaCommand.Action.WEEKLY_REVIEW -> openOverlay(Overlay.REVIEW)
                    ...
                    OmegaCommand.Action.RECAP_WEEK -> { val ws = com.wkhan.hexis.domain.weekStartOf(now, settings.weekStart); argOverlay = OverlayArg.Recap(ws.toEpochDay(), td, "This week") }
                    OmegaCommand.Action.RECAP_LAST_WEEK -> { val ws = com.wkhan.hexis.domain.weekStartOf(now, settings.weekStart); argOverlay = OverlayArg.Recap(ws.minusWeeks(1).toEpochDay(), ws.minusDays(1).toEpochDay(), "Last week") }
                ...
                is OmegaCommand.Command.Goto -> {
                    ...
                    val overlayByName: Map<String, () -> Unit> = mapOf(
                        ...
                        "goals hub" to { openOverlay(Overlay.GOALS) }, "my goals" to { openOverlay(Overlay.GOALS) }, "goal" to { openOverlay(Overlay.GOALS) },
                        ...
                        "journal" to { argOverlay = OverlayArg.Journal(com.wkhan.hexis.domain.PeriodRange.DAY, now.toEpochDay()) }, "periodic notes" to { argOverlay = OverlayArg.Journal(com.wkhan.hexis.domain.PeriodRange.DAY, now.toEpochDay()) },
                        ...
                        "weekly note" to { argOverlay = OverlayArg.Journal(com.wkhan.hexis.domain.PeriodRange.WEEK, now.toEpochDay()) },
                        "monthly note" to { argOverlay = OverlayArg.Journal(com.wkhan.hexis.domain.PeriodRange.MONTH, now.toEpochDay()) },
                        "yearly note" to { argOverlay = OverlayArg.Journal(com.wkhan.hexis.domain.PeriodRange.YEAR, now.toEpochDay()) },
        ...
        if (settingsLoaded && !settings.onboardedModules) com.wkhan.hexis.ui.screens.ModulePickerDialog(
        ...
        if (saveTab) {
            ...
            AlertDialog(
                ...
                text = { com.wkhan.hexis.ui.components.AppTextField(tabName, { tabName = it }, singleLine = true, label = { Text("Tab name") }, modifier = Modifier.fillMaxWidth()) },
            )
        }
        if (templatePicker) {
            val templates by vm.templates.collectAsStateWithLifecycle()
            var renaming by remember { mutableStateOf<com.wkhan.hexis.data.entity.TemplateEntity?>(null) }
            AlertDialog(
            ...
            renaming?.let { t ->
                ...
                AlertDialog(
                    ...
                    text = { com.wkhan.hexis.ui.components.AppTextField(nm, { nm = it }, singleLine = true, modifier = Modifier.fillMaxWidth()) },
                )
            }
        }
...
private fun FilterBuilderDialog(
    filter: com.wkhan.hexis.data.entity.FilterEntity,
    lists: List<ListEntity>, folders: List<FolderEntity>, tags: List<com.wkhan.hexis.data.entity.TagEntity>, contexts: List<com.wkhan.hexis.data.entity.ContextEntity>,
    flags: List<com.wkhan.hexis.data.entity.FlagEntity>,
    onDismiss: () -> Unit, onDelete: () -> Unit, onSave: (com.wkhan.hexis.data.entity.FilterEntity) -> Unit,
) {
    val q0 = com.wkhan.hexis.domain.view.Filters.parse(filter.queryJson)
    ...
    fun save() {
        val q = com.wkhan.hexis.domain.view.FilterQuery(
        ...
        onSave(filter.copy(name = name.trim().ifBlank { "Filter" }, queryJson = com.wkhan.hexis.domain.view.Filters.encode(q)))
    }
    AlertDialog(
        ...
        text = {
            ...
            Column(Modifier.heightIn(max = 460.dp).verticalScroll(sc)) {
                com.wkhan.hexis.ui.components.AppTextField(name, { name = it }, singleLine = true, label = { Text("Name") }, modifier = Modifier.fillMaxWidth())
...
private fun ViewTabStrip(vm: AppViewModel) {
    ...
    var renaming by remember { mutableStateOf<com.wkhan.hexis.domain.view.ViewTab?>(null) }
    Row(
        Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()).padding(horizontal = 10.dp, vertical = 5.dp),
        horizontalArrangement = Arrangement.spacedBy(7.dp),
    ) {
        tabs.forEach { t ->
            val active = com.wkhan.hexis.domain.view.ViewTabs.viewOf(t.ref) == current
            Box {
                androidx.compose.material3.Surface(
...
@Composable
private fun ManageWorkspaceDialog(w: com.wkhan.hexis.data.entity.WorkspaceEntity, onDismiss: () -> Unit, onRename: (String) -> Unit, onDelete: () -> Unit) {
    var name by remember { mutableStateOf(w.name) }
    val isDefault = w.id == com.wkhan.hexis.data.entity.WorkspaceEntity.DEFAULT_ID
    AlertDialog(
        onDismissRequest = onDismiss,
        ...
        text = {
            Column {
                com.wkhan.hexis.ui.components.AppTextField(name, { name = it }, singleLine = true, modifier = Modifier.fillMaxWidth())
...
private fun ctxDescendantsOf(id: String, all: List<com.wkhan.hexis.data.entity.ContextEntity>): Set<String> {
...
private fun tagDescendantsOf(id: String, tags: List<com.wkhan.hexis.data.entity.TagEntity>): Set<String> {
    val out = mutableSetOf<String>()
    var frontier = listOf(id)
    while (frontier.isNotEmpty()) {
    ...
    AlertDialog(
        ...
        text = {
            Column {
                ...
                Spacer(Modifier.size(10.dp))
                com.wkhan.hexis.ui.components.AppTextField(name, { name = it }, placeholder = { Text("Name") }, singleLine = true, modifier = Modifier.fillMaxWidth())
...
private fun ManageListDialog(
    ...
    AlertDialog(
        ...
        text = {
            Column {
                com.wkhan.hexis.ui.components.AppTextField(name, { name = it }, singleLine = true, modifier = Modifier.fillMaxWidth(), label = { Text("Name") })
                Spacer(Modifier.size(8.dp))
                com.wkhan.hexis.ui.components.AppTextField(description, { description = it }, modifier = Modifier.fillMaxWidth(), label = { Text("Description (optional)") }, minLines = 2)
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    com.wkhan.hexis.ui.components.AppColorPicker(current = list.colorArgb, onPick = { it?.let(onColor) }, allowNone = false)
                }
                Spacer(Modifier.size(12.dp))
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    TextButton(onClick = { com.wkhan.hexis.util.SystemPicker.galleryOne(onError = { android.widget.Toast.makeText(bgCtxTop, it, android.widget.Toast.LENGTH_LONG).show() }) { onPickBackground(it) } }) { Text(if (list.backgroundBase64 == null) "Set image" else "Change image") }
...
private fun EmojiPicker(current: String?, onPick: (String?) -> Unit) {
    com.wkhan.hexis.ui.components.EmojiGridPicker(current = current, onPick = onPick)
}
...
private fun DeleteContainerDialog(
    ...
    if (showMovePicker) {
        com.wkhan.hexis.ui.screens.MoveTargetDialog(
        ...
        text = {
            Column {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Spacer(Modifier.size(10.dp))
                    com.wkhan.hexis.ui.components.AppColorPicker(current = folder.colorArgb, onPick = onColor, allowNone = true)
                }
                Spacer(Modifier.size(4.dp))
...
private fun TextEntryDialog(title: String, placeholder: String, onDismiss: () -> Unit, onConfirm: (String) -> Unit) {
    ...
    AlertDialog(
        ...
        dismissButton = { TextButton(onClick = onDismiss) { Text("Cancel") } },
        title = { Text(title) },
        text = { com.wkhan.hexis.ui.components.AppTextField(name, { name = it }, placeholder = { Text(placeholder) }, singleLine = true, modifier = Modifier.fillMaxWidth()) },
    )
}

@Composable
private fun ManageTagDialog(tag: com.wkhan.hexis.data.entity.TagEntity, onDismiss: () -> Unit, onRename: (String) -> Unit, onColor: (Long?) -> Unit, onDelete: () -> Unit) {
    ...
    AlertDialog(
        ...
        text = {
            Column {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Text("Colour", Modifier.weight(1f), style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    com.wkhan.hexis.ui.components.AppColorPicker(current = tag.colorArgb, onPick = { onColor(it) }, allowNone = true)
                }
            }
        },
    )
...
@Composable
private fun TagPickerDialog(title: String, tags: List<com.wkhan.hexis.data.entity.TagEntity>, exclude: Set<String>, onDismiss: () -> Unit, onPick: (String?) -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
        confirmButton = { TextButton(onClick = onDismiss) { Text("Cancel") } },
        title = { Text(title) },
...
private fun ManageContextDialog(
    ctx: com.wkhan.hexis.data.entity.ContextEntity, onDismiss: () -> Unit,
    ...
    val oh0 = com.wkhan.hexis.domain.context.ContextAvailability.parse(ctx.openHoursJson)
    ...
    fun persistHours() {
        onHours(if (restricted) com.wkhan.hexis.domain.context.ContextAvailability.encode(
            com.wkhan.hexis.domain.context.OpenHours(days, startH * 60, endH * 60)) else null)
    }
    AlertDialog(
        onDismissRequest = onDismiss,
        confirmButton = { TextButton(onClick = { if (name.isNotBlank()) onRename(name.trim()) }) { Text("Save") } },
        ...
        text = {
            Column {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Text("Colour", Modifier.weight(1f), style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    com.wkhan.hexis.ui.components.AppColorPicker(current = ctx.colorArgb, onPick = { onColor(it) }, allowNone = true)
                }
                ...
                if (restricted) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        com.wkhan.hexis.ui.components.TimeChip(startH * 60, onPick = { m -> startH = ((m + 30) / 60).coerceIn(0, endH); persistHours() })
                        Spacer(Modifier.size(10.dp))
                        Text("to", Modifier.padding(end = 6.dp))
                        com.wkhan.hexis.ui.components.TimeChip((endH % 24) * 60, onPick = { m -> endH = ((m + 30) / 60).let { if (it == 0) 24 else it }.coerceIn(startH, 24); persistHours() })
                    }
                }
            }
        },
...
@Composable
private fun ContextPickerDialog(title: String, all: List<com.wkhan.hexis.data.entity.ContextEntity>, exclude: Set<String>, onDismiss: () -> Unit, onPick: (String?) -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
        confirmButton = { TextButton(onClick = onDismiss) { Text("Cancel") } },
        title = { Text(title) },
