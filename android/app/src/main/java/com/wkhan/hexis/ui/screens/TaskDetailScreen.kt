...
import com.wkhan.hexis.ui.components.priorityColor
...
import com.wkhan.hexis.ui.components.CardLabel
...
import com.wkhan.hexis.ui.components.DateTimePickerDialog
import com.wkhan.hexis.ui.components.formatDue
import com.wkhan.hexis.ui.components.formatDueSpan
...
fun TaskDetailScreen(vm: AppViewModel, taskId: String, onBack: () -> Unit, onJustStart: ((String) -> Unit)? = null, onOpenTask: ((String) -> Unit)? = null, onOpenNote: ((String) -> Unit)? = null) {
    ...
    var editActivity by remember { mutableStateOf<com.wkhan.hexis.data.entity.TimeActivityEntity?>(null) }
    ...
    var pendingDeleteAtt by remember(taskId) { mutableStateOf<com.wkhan.hexis.data.entity.AttachmentMeta?>(null) }
    ...
    Scaffold(topBar = {
        TopAppBar(expandedHeight = 52.dp, 
            ...
            actions = {
                ...
                if (onJustStart != null && task != null && !task.completed && !task.abandoned &&
                    !com.wkhan.hexis.domain.Modules.isEnabled(settings, com.wkhan.hexis.domain.Modules.TIME)) {
        ...
        Column(
            ...
            Row(verticalAlignment = Alignment.Top) {
                com.wkhan.hexis.ui.components.PriorityCheckbox(task.completed, plevel, onCheckedChange = {
            ...
            Box(Modifier.fillMaxWidth().padding(end = 4.dp)) {
                if (task.note.isNotBlank() && notePreview) {
                    ...
                    androidx.compose.foundation.text.selection.SelectionContainer {
                        com.wkhan.hexis.ui.components.RichMarkdown(
                            task.note,
            ...
            if (onOpenNote != null && task.title.isNotBlank()) {
                val noteBacklinks = allNotes.filter { !it.trashed && com.wkhan.hexis.domain.NoteLinks.links(it.body, task.title) }
            ...
            fun hasFieldValue(f: com.wkhan.hexis.domain.EditorField): Boolean = when (f) {
                com.wkhan.hexis.domain.EditorField.PROGRESS -> totalN > 0 || (task.progressPct ?: 0) > 0
                com.wkhan.hexis.domain.EditorField.SCHEDULE -> task.dueDate != null || task.startDate != null || task.deadlineDate != null
                ...
                com.wkhan.hexis.domain.EditorField.LEADTIME -> task.leadTimeMin != null
                ...
                com.wkhan.hexis.domain.EditorField.TIMETRACKING -> timeEntries.any { it.taskId == task.id }
                com.wkhan.hexis.domain.EditorField.PRIORITY -> true
                com.wkhan.hexis.domain.EditorField.LIST -> true
                com.wkhan.hexis.domain.EditorField.REPEAT -> !task.rrule.isNullOrBlank()
                com.wkhan.hexis.domain.EditorField.CHECKLIST -> myCheck.isNotEmpty()
                com.wkhan.hexis.domain.EditorField.SUBTASKS -> hasChildren
                com.wkhan.hexis.domain.EditorField.ENERGY -> task.energy != null
                com.wkhan.hexis.domain.EditorField.FLAG -> task.flagId != null
                com.wkhan.hexis.domain.EditorField.ATTACHMENTS -> attachments.isNotEmpty()
                com.wkhan.hexis.domain.EditorField.TAGS -> assignedTags.isNotEmpty() || assignedCtx.isNotEmpty()
                com.wkhan.hexis.domain.EditorField.BLOCKED -> myDeps.isNotEmpty()
                com.wkhan.hexis.domain.EditorField.ACTIVITY -> activityLog.isNotEmpty()
                com.wkhan.hexis.domain.EditorField.ADVANCED -> task.estimateMin != null || task.isGoal || task.isProject || task.reviewEveryDays != null || (task.progressPct ?: 0) > 0
                com.wkhan.hexis.domain.EditorField.REFLECTION -> task.winFlag || !task.outcomeNote.isNullOrBlank() || !task.learnedNote.isNullOrBlank() || !task.praiseQuote.isNullOrBlank() || task.mood != null
                com.wkhan.hexis.domain.EditorField.COACH -> true
            }
            val orderedFields = settings.editorFieldsOrdered()
            var moreExpanded by remember(task.id) { mutableStateOf(false) }
            val anyCollapsed = orderedFields.any { it != com.wkhan.hexis.domain.EditorField.COACH && settings.editorTier(it) == com.wkhan.hexis.domain.AppSettings.TIER_MORE && !hasFieldValue(it) }
            ...
            orderedFields.forEach { f ->
                ...
                if (f == com.wkhan.hexis.domain.EditorField.COACH) return@forEach
                val tier = settings.editorTier(f)
                val visible = tier == com.wkhan.hexis.domain.AppSettings.TIER_ALWAYS || hasFieldValue(f) || (tier == com.wkhan.hexis.domain.AppSettings.TIER_MORE && moreExpanded) ||
                    // Reflection still auto-appears on a finished task — unless the user hid it.
                    (f == com.wkhan.hexis.domain.EditorField.REFLECTION && task.completed && tier != com.wkhan.hexis.domain.AppSettings.TIER_HIDDEN)
                ...
                when (f) {
                    ...
                    com.wkhan.hexis.domain.EditorField.PROGRESS -> {
                        if (totalN > 0) {
                    ...
                    com.wkhan.hexis.domain.EditorField.SCHEDULE -> {
                    ...
                    com.wkhan.hexis.domain.EditorField.LEADTIME -> {
                    ...
                    com.wkhan.hexis.domain.EditorField.TIMETRACKING -> if (timeOn) {
                    ...
                    com.wkhan.hexis.domain.EditorField.PRIORITY -> {
                        Row(
                    ...
                    // ---------- List / folder ----------
                    com.wkhan.hexis.domain.EditorField.LIST -> {
                        run {
                    ...
                    com.wkhan.hexis.domain.EditorField.REPEAT -> {
                        ...
                        reliability[task.id]?.let { rel ->
                            ...
                            val trend = remember(acts, task.rrule) { com.wkhan.hexis.domain.task.TaskReliability.trend(task, acts, System.currentTimeMillis()) }
                            val hours = remember(acts, task.rrule) { com.wkhan.hexis.domain.task.TaskReliability.completionHours(task, acts) }
                    ...
                    com.wkhan.hexis.domain.EditorField.CHECKLIST ->
                ...
                if (showBreakdown) {
                    ...
                    AlertDialog(
                        ...
                        text = {
                            Column {
                                ...
                                com.wkhan.hexis.ui.components.AppTextField(bulk, { bulk = it }, modifier = Modifier.fillMaxWidth().heightIn(min = 120.dp), placeholder = { Text("Draft outline\nGather sources\nWrite first pass\nEdit") })
                            }
                        },
                    )
                }
                    ...
                    com.wkhan.hexis.domain.EditorField.SUBTASKS -> {
                        ...
                        DetailSection("Subtasks", if (children.isEmpty()) null else "${children.count { it.completed }}/${children.size}", !fldFolded) {
                            ...
                            children.forEach { child ->
                                ...
                                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                                    com.wkhan.hexis.ui.components.PriorityCheckbox(child.completed, cl, onCheckedChange = { vm.toggleComplete(child); autoBump++ }, onSetLevel = { lvl -> vm.setPriority(child, lvl); autoBump++ })
                    ...
                    com.wkhan.hexis.domain.EditorField.ATTACHMENTS ->
                ...
                pendingDeleteAtt?.let { a ->
                    ...
                    com.wkhan.hexis.domain.EditorField.TAGS ->
                ...
                AddInline(newContext, { newContext = it }, "New context") { if (it.isNotBlank()) { vm.createContext(it.trim()); newContext = "" } }
                    ...
                    com.wkhan.hexis.domain.EditorField.FLAG ->
                        Box {
                            ...
                            DropdownMenu(expanded = flagMenu, onDismissRequest = { flagMenu = false }) {
                                ...
                                allFlags.forEach { fl ->
                                    DropdownMenuItem(text = { Text(fl.name) }, leadingIcon = { Icon(com.wkhan.hexis.ui.components.FlagIcons.vector(fl.icon), null, tint = Color(fl.colorArgb), modifier = Modifier.size(18.dp)) },
                                        onClick = { update { it.copy(flagId = fl.id, flagColorArgb = fl.colorArgb) }; flagMenu = false })
                                }
                            }
                        }

                    com.wkhan.hexis.domain.EditorField.ENERGY ->
                    ...
                    com.wkhan.hexis.domain.EditorField.BLOCKED -> {
                ...
                myDeps.forEach { dep ->
                    ...
                    Row(Modifier.fillMaxWidth().padding(vertical = 4.dp), verticalAlignment = Alignment.CenterVertically) {
                        Icon(if (pred?.completed == true) Icons.Filled.CheckCircle else Icons.Filled.RadioButtonUnchecked, null,
                            tint = if (pred?.completed == true) LocalHexisColors.current.good else MaterialTheme.colorScheme.outline, modifier = Modifier.size(18.dp))
                        Spacer(Modifier.width(8.dp))
                ...
                if (waitingParties.isNotEmpty()) {
                    ...
                    Text(
                        ...
                        style = MaterialTheme.typography.labelSmall,
                        color = if (days >= 14L) LocalHexisColors.current.bad else LocalHexisColors.current.warn,
                    )
                }
            } }

                    com.wkhan.hexis.domain.EditorField.ADVANCED ->
                ...
                if (task.isGoal || task.isProject) {
                    com.wkhan.hexis.ui.components.AppTextField(task.whyText, { v -> update { it.copy(whyText = v.take(140)) } }, singleLine = false,
                        label = { Text("Why this matters (shown when you open it)") }, modifier = Modifier.fillMaxWidth().padding(top = 6.dp))
                    com.wkhan.hexis.ui.components.AppTextField(task.rewardText, { v -> update { it.copy(rewardText = v.take(80)) } }, singleLine = true,
                ...
                if (task.reviewEveryDays != null) {
                    ...
                    com.wkhan.hexis.domain.EditorField.ACTIVITY ->
                ...
                var confirmDel by remember { mutableStateOf<com.wkhan.hexis.data.entity.ActivityEntity?>(null) }
                ...
                if (confirmClear) ConfirmDialog(
                    ...
                    com.wkhan.hexis.domain.EditorField.REFLECTION -> {
                        ...
                        DetailSection("Reflection", if (task.winFlag) "★" else null, !fldFolded) {
                            ...
                            Spacer(Modifier.height(8.dp))
                            com.wkhan.hexis.ui.components.AppTextField(task.outcomeNote ?: "", { v -> update { it.copy(outcomeNote = v) } },
                                label = { Text("Outcome / impact") }, modifier = Modifier.fillMaxWidth())
                            Spacer(Modifier.height(6.dp))
                            com.wkhan.hexis.ui.components.AppTextField(task.learnedNote ?: "", { v -> update { it.copy(learnedNote = v) } },
                                label = { Text("What I learned") }, modifier = Modifier.fillMaxWidth())
                            Spacer(Modifier.height(6.dp))
                            com.wkhan.hexis.ui.components.AppTextField(task.praiseQuote ?: "", { v -> update { it.copy(praiseQuote = v) } },
                                label = { Text("Praise / thank-you to remember") }, modifier = Modifier.fillMaxWidth())
                        }
                    }

                    com.wkhan.hexis.domain.EditorField.COACH -> {}
                }
            }
            }
            ...
            if (settings.editorTier(com.wkhan.hexis.domain.EditorField.COACH) != com.wkhan.hexis.domain.AppSettings.TIER_HIDDEN) TaskCoachCard(vm, task)
    ...
    if (showDue) {
        ...
        com.wkhan.hexis.ui.components.DateReminderSheet(
    ...
    if (prioSheet && task != null) com.wkhan.hexis.ui.components.PrioritySheet(
            ...
            text = {
                Column {
                    Text("Saves this task and its subtree — note, priority, flag, checklist, tags, contexts, recurrence and relative dates — for reuse.",
                        style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    Spacer(Modifier.height(10.dp))
                    com.wkhan.hexis.ui.components.AppTextField(tplName, { tplName = it }, singleLine = true, label = { Text("Template name") }, modifier = Modifier.fillMaxWidth())
                }
            },
        )
    }
...
private fun ActivityEditDialog(
    activity: com.wkhan.hexis.data.entity.TimeActivityEntity,
    onDismiss: () -> Unit,
    onSave: (com.wkhan.hexis.data.entity.TimeActivityEntity) -> Unit,
    onDelete: () -> Unit,
) {
    ...
    AlertDialog(
        ...
        text = {
            Column(Modifier.verticalScroll(rememberScrollState())) {
                ...
                com.wkhan.hexis.ui.components.EmojiGridPicker(emoji) { emoji = it }
                Spacer(Modifier.height(8.dp))
    ...
    Column {
        myReminders.forEach { r ->
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                val tier = com.wkhan.hexis.domain.reminders.ReminderPresets.tierOf(r.annoying, r.escalate)
                val tint = when (tier) {
                ...
                TextButton(onClick = { vm.setReminderTier(r, task, (tier + 1) % 3); onChanged() }, contentPadding = androidx.compose.foundation.layout.PaddingValues(horizontal = 8.dp)) {
                    ...
                    Spacer(Modifier.width(4.dp))
                    Text(com.wkhan.hexis.domain.reminders.ReminderPresets.TIER_LABELS[tier], style = MaterialTheme.typography.labelMedium, color = tint)
                }
        ...
        Box {
            ...
            DropdownMenu(expanded = addMenu, onDismissRequest = { addMenu = false }) {
                ...
                if (task.dueDate != null) {
                    HorizontalDivider()
                    com.wkhan.hexis.domain.reminders.ReminderPresets.OFFSETS.forEach { off ->
                        val label = if (off == 0) "When due" else "${com.wkhan.hexis.domain.reminders.ReminderPresets.beforeLabel(off)} due"
                ...
                if (task.startDate != null) {
                    HorizontalDivider()
                    com.wkhan.hexis.domain.reminders.ReminderPresets.OFFSETS.forEach { off ->
                        val label = if (off == 0) "When it starts" else "${com.wkhan.hexis.domain.reminders.ReminderPresets.beforeLabel(off)} start"
                ...
                if (task.deadlineDate != null) {
                    ...
                    listOf(0, 60, 1440, 4320).forEach { off ->
                        val label = if (off == 0) "At deadline" else "${com.wkhan.hexis.domain.reminders.ReminderPresets.beforeLabel(off)} deadline"
            ...
            if (placeDialog) {
                ...
                AlertDialog(
                    ...
                    text = {
                        Column {
                            com.wkhan.hexis.ui.components.AppTextField(place, { place = it }, singleLine = true, label = { Text("Place name (e.g. Office, Home)") }, modifier = Modifier.fillMaxWidth())
                            Spacer(Modifier.height(10.dp))
                            Text("Fully offline — the app never tracks your location. Trigger it when you arrive by scanning an NFC tag or QR code you place there, encoding:",
                                style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                            Text("hexis://arrive?place=${place.trim().ifBlank { "Office" }}",
...
fun fmtDuration(min: Int): String = com.wkhan.hexis.util.formatMinutes(min)
...
@Composable
private fun TaskCoachCard(vm: AppViewModel, task: com.wkhan.hexis.data.entity.TaskEntity) {
    ...
    val lesson = remember(task, subtaskCount, hour) { com.wkhan.hexis.domain.task.TaskCoach.taskLesson(task, subtaskCount, hour, now) }
    val reliability = remember(task, revisions) { com.wkhan.hexis.domain.task.TaskCoach.reliability(task, revisions) }
        ...
        AlertDialog(onDismissRequest = { escrowOpen = false },
            ...
            text = {
                Column {
                    ...
                    Spacer(Modifier.height(8.dp))
                    com.wkhan.hexis.ui.components.AppTextField(desc, { desc = it }, singleLine = true, label = { Text("Reward or stake") }, modifier = Modifier.fillMaxWidth())
                    Spacer(Modifier.height(8.dp))
