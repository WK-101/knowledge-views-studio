...
object Export {
    ...
    fun toHabitsCsv(
        ...
        checkins.sortedWith(compareBy({ it.habitId }, { it.epochDay })).forEach { c ->
            ...
            val row = listOf(h.name, h.habitType, h.unit ?: "", h.targetPerDay.toString(),
                com.wkhan.hexis.domain.habit.HabitStats.frequencyLabel(h), date, c.count.toString(), c.status)
            sb.append(row.joinToString(",") { esc(it) }).append('\n')
        }
        return sb.toString()
    }
    ...
    fun toIcs(
        ...
        line("VERSION:2.0")
        line("PRODID:-//Hexis//Tasks//EN")
        line("CALSCALE:GREGORIAN")
        line("METHOD:PUBLISH")
        line("X-WR-CALNAME:Hexis")
