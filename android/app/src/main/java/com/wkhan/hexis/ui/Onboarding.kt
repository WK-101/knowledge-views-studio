...
import com.wkhan.hexis.ui.components.HexisMark
            ...
            HorizontalPager(
                ...
                Column(Modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally) {
                    ...
                    Box(Modifier.fillMaxWidth().height(172.dp), contentAlignment = Alignment.Center) {
                        if (p.brand) {
                            // Render the Hexis mark ("four into one") by DRAWING it with a Compose Canvas —
                            // no resource is loaded at all, so this page can never crash on a drawable parse.
                            // Theme-aware: the tile and mark follow the app's selected palette and accent.
                            Box(
                                ...
                                HexisMark(
                                    ...
                                    stroke = MaterialTheme.colorScheme.onPrimary,
                                    core = MaterialTheme.colorScheme.onPrimary,
                                )
                            }
                        } else {
