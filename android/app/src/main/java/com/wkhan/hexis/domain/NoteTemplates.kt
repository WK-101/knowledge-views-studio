 ...
 * plus [transclusion][com.wkhan.hexis.util.NoteTransclusion] tokens ({{today}}/{{events}}/{{habits}}/
 * {{tasks}}) that recompute every time the note opens, and date/time tokens ([NoteTokens]) resolved once
 * at apply time. NotePlan templates a note; Amplenote templates a task; only Hexis can template a note
 * that assembles itself from your tasks, events and habits — because only Hexis has all of them.
 *
...
object NoteTemplates {
    ...
    private val json = com.wkhan.hexis.util.AppJson
