...
fun RepeatDialog(rule: String?, hasChildren: Boolean, onDismiss: () -> Unit, onSave: (String?) -> Unit) {
    val r0 = com.wkhan.hexis.domain.recurrence.Recurrence.parse(rule)
    ...
    fun build(): String? {
        ...
        val isMonthly = f == com.wkhan.hexis.domain.recurrence.Freq.MONTHLY
        return com.wkhan.hexis.domain.recurrence.Recurrence.encode(
            com.wkhan.hexis.domain.recurrence.Recur(
                freq = f, interval = interval.coerceAtLeast(1),
                byDays = if (f == com.wkhan.hexis.domain.recurrence.Freq.WEEKLY) days else emptySet(),
    ...
    AlertDialog(
        ...
        text = {
            Column {
                val freqs = listOf<Pair<com.wkhan.hexis.domain.recurrence.Freq?, String>>(
                    null to "None",
                    com.wkhan.hexis.domain.recurrence.Freq.DAILY to "Daily",
                    com.wkhan.hexis.domain.recurrence.Freq.WEEKDAYS to "Weekday",
                    com.wkhan.hexis.domain.recurrence.Freq.WEEKLY to "Weekly",
                    com.wkhan.hexis.domain.recurrence.Freq.MONTHLY to "Monthly",
                    com.wkhan.hexis.domain.recurrence.Freq.YEARLY to "Yearly",
                )
                OptionChips(freqs.map { it.first }, freq, { freq = it }, spacing = 6) { f -> freqs.first { it.first == f }.second }
                if (freq != null && freq != com.wkhan.hexis.domain.recurrence.Freq.WEEKDAYS) {
                ...
                if (freq == com.wkhan.hexis.domain.recurrence.Freq.WEEKLY) {
                    Spacer(Modifier.size(8.dp))
                ...
                if (freq == com.wkhan.hexis.domain.recurrence.Freq.MONTHLY) {
                    Spacer(Modifier.size(8.dp))
