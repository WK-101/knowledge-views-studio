...
import com.wkhan.hexis.ui.AppRoot
import com.wkhan.hexis.util.SystemPicker
...
class MainActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            (application as App).repository.allSettings.collect { rows ->
                ...
                if (flag(com.wkhan.hexis.domain.AppSettings.Keys.SECURE_SCREEN) ||
                    flag(com.wkhan.hexis.domain.AppSettings.Keys.APP_LOCK)) window.addFlags(WindowManager.LayoutParams.FLAG_SECURE)
                else window.clearFlags(WindowManager.LayoutParams.FLAG_SECURE)
                com.wkhan.hexis.reminders.Notifications.lockscreenPrivate = flag(com.wkhan.hexis.domain.AppSettings.Keys.LOCKSCREEN_PRIVACY)
                // R59 — keep the notification Snooze duration in sync with the setting.
                rows.firstOrNull { it.key == com.wkhan.hexis.domain.AppSettings.Keys.SNOOZE_MIN }?.value?.toIntOrNull()
                    ?.let { com.wkhan.hexis.reminders.Notifications.snoozeMinutes = it.coerceIn(1, 720) }
                // R59 (Wave 2) — keep quiet hours in sync.
                com.wkhan.hexis.reminders.AlarmScheduler.quietEnabled = flag(com.wkhan.hexis.domain.AppSettings.Keys.QUIET_ON)
                rows.firstOrNull { it.key == com.wkhan.hexis.domain.AppSettings.Keys.QUIET_START }?.value?.toIntOrNull()?.let { com.wkhan.hexis.reminders.AlarmScheduler.quietStartHour = it.coerceIn(0, 23) }
    ...
    private fun resolveImport(intent: Intent?): Uri? {
        ...
        if (intent.action == Intent.ACTION_VIEW && intent.data?.scheme == "hexis") return null
    ...
    private fun resolveAction(intent: Intent?): String? {
        if (intent == null) return null
        // U13: an NFC tag or QR encoding hexis://track?activity=<id> (or ?name=<name>) starts a timer.
        if (intent.action == Intent.ACTION_VIEW) {
            val d = intent.data
            if (d?.scheme == "hexis" && d.host == "track") {
            ...
            if (d?.scheme == "hexis" && d.host == "routine") {
    ...
    companion object {
        ...
        const val EXTRA_ACTION = "com.wkhan.hexis.action"
