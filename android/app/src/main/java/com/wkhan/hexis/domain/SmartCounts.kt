...
object SmartCounts {
    fun compute(
        ...
        val active = if (hiddenListIds.isEmpty() && hiddenFolderIds.isEmpty()) wsTasks
            else wsTasks.filterNot { com.wkhan.hexis.domain.view.ListPipeline.isHiddenContainerTask(it, hiddenListIds, hiddenFolderIds) }
        return SmartKind.entries.associateWith { k ->
            when (k) {
                ...
                SmartKind.DO_NEXT -> DoNext.focused(wsTasks, now, prioCfg, deps, tcRefs, ctxs, null, null, zone, dayStartMin)
                    .count { !com.wkhan.hexis.domain.view.ListPipeline.isHiddenContainerTask(it, hiddenListIds, hiddenFolderIds) }
