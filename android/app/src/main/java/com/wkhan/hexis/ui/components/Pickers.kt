            ...
            if (reminderSlot != null) {
                ...
                if (openSection == "reminder") {
                    FlowRow(Modifier.padding(start = 34.dp, top = 2.dp, bottom = 4.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        (listOf<Pair<Int?, String>>(null to "None") + com.wkhan.hexis.domain.reminders.ReminderPresets.OFFSETS.map { m -> m to (if (m == 0) "On time" else com.wkhan.hexis.domain.reminders.ReminderPresets.beforeLabel(m)) }).forEach { (off, lbl) ->
...
internal fun fmtDuration(min: Int): String = com.wkhan.hexis.util.formatMinutes(min)
