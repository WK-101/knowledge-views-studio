...
class CalendarViewModel(
    ...
    fun setEventCalendarVisible(c: com.wkhan.hexis.data.entity.EventCalendarEntity, visible: Boolean) = scope.launch { repo.upsertEventCalendar(c.copy(visible = visible)) }
    ...
    fun renameEventCalendar(c: com.wkhan.hexis.data.entity.EventCalendarEntity, name: String, color: Long) = scope.launch { repo.upsertEventCalendar(c.copy(name = name.trim().ifBlank { c.name }, colorArgb = color)) }
    fun deleteEventCalendar(id: String) = scope.launch {
        val evs = repo.eventsOnce().filter { it.calendarId == id }
        evs.forEach { com.wkhan.hexis.reminders.AlarmScheduler.cancelEventAlerts(app.appCtx, it); repo.deleteEvent(it.id) }
    ...
    fun saveEvent(existingId: String?, calendarId: String, title: String, location: String, notes: String, url: String,
        ...
        old?.let { com.wkhan.hexis.reminders.AlarmScheduler.cancelEventAlerts(app.appCtx, it) }
        val e = (old ?: com.wkhan.hexis.data.entity.EventEntity(
        ...
        repo.upsertEvent(e)
        com.wkhan.hexis.reminders.AlarmScheduler.scheduleEventAlerts(app.appCtx, e)
    }
    ...
    fun quickAddFromCalendar(text: String, anchorMillis: Long) = scope.launch {
        ...
        val draft = com.wkhan.hexis.domain.calendar.EventParser.parse(raw, anchorDay, zone)
    ...
    private fun capUntil(rule: String, untilDay: Long): String {
        val r = com.wkhan.hexis.domain.recurrence.Recurrence.parse(rule) ?: return rule
        return com.wkhan.hexis.domain.recurrence.Recurrence.encode(r.copy(untilEpochDay = untilDay, count = null))
    }
