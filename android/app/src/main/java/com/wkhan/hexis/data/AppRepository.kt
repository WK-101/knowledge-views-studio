...
import com.wkhan.hexis.data.entity.TemplateTask
...
import com.wkhan.hexis.domain.port.Backup
import com.wkhan.hexis.domain.port.BackupFile
import com.wkhan.hexis.data.security.SecurePrefs
...
class AppRepository(private val db: AppDatabase, private val appContext: android.content.Context? = null) {
    ...
    private val templateJson = com.wkhan.hexis.util.AppJson
    ...
    private suspend fun maybeRecordRevision(t: TaskEntity) {
        ...
        runCatching {
            revisions.insert(
                com.wkhan.hexis.data.entity.TaskRevisionEntity(
    ...
    fun taskRevisions(taskId: String): Flow<List<com.wkhan.hexis.data.entity.TaskRevisionEntity>> = revisions.observeForTask(taskId)
    ...
    fun taskActivity(taskId: String): Flow<List<com.wkhan.hexis.data.entity.ActivityEntity>> = activity.observeForTask(taskId)
    val allActivity: Flow<List<com.wkhan.hexis.data.entity.ActivityEntity>> = activity.observeAll()
    suspend fun getActivitiesOnce(): List<com.wkhan.hexis.data.entity.ActivityEntity> = activity.getAll()
    ...
    private suspend fun logActivity(taskId: String, type: String, detail: String? = null) {
        activity.insert(com.wkhan.hexis.data.entity.ActivityEntity(uid(), taskId, type, now(), detail))
    }
    ...
    val allCountdowns: Flow<List<com.wkhan.hexis.data.entity.CountdownEntity>> = countdowns.observeAll()
    suspend fun upsertCountdown(c: com.wkhan.hexis.data.entity.CountdownEntity) = countdowns.upsert(c)
    suspend fun deleteCountdown(id: String) = countdowns.deleteById(id)
    val allSealedNotes: Flow<List<com.wkhan.hexis.data.entity.SealedNoteEntity>> = sealedNotes.observeAll()
    suspend fun upsertSealedNote(n: com.wkhan.hexis.data.entity.SealedNoteEntity) = sealedNotes.upsert(n)
    ...
    suspend fun allSealedNotesOnce(): List<com.wkhan.hexis.data.entity.SealedNoteEntity> = sealedNotes.getAll()
    suspend fun sealedNoteById(id: String): com.wkhan.hexis.data.entity.SealedNoteEntity? =
    ...
    val allCravings: Flow<List<com.wkhan.hexis.data.entity.CravingEventEntity>> = cravings.observeAll()
    suspend fun upsertCraving(c: com.wkhan.hexis.data.entity.CravingEventEntity) = cravings.upsert(c)
    suspend fun deleteCraving(id: String) = cravings.deleteById(id)
    // R34 — life-systems layer accessors.
    val allCoreValues: Flow<List<com.wkhan.hexis.data.entity.CoreValueEntity>> = coreValues.observeAll()
    suspend fun upsertCoreValue(v: com.wkhan.hexis.data.entity.CoreValueEntity) = coreValues.upsert(v)
    suspend fun deleteCoreValue(id: String) = coreValues.deleteById(id)
    val allWitnessEvents: Flow<List<com.wkhan.hexis.data.entity.WitnessEventEntity>> = witnesses.observeAll()
    suspend fun upsertWitness(w: com.wkhan.hexis.data.entity.WitnessEventEntity) = witnesses.upsert(w)
    suspend fun deleteWitness(id: String) = witnesses.deleteById(id)
    val allScorecardItems: Flow<List<com.wkhan.hexis.data.entity.ScorecardItemEntity>> = scorecard.observeAll()
    suspend fun upsertScorecardItem(s: com.wkhan.hexis.data.entity.ScorecardItemEntity) = scorecard.upsert(s)
    suspend fun deleteScorecardItem(id: String) = scorecard.deleteById(id)
    val allBuddies: Flow<List<com.wkhan.hexis.data.entity.BuddySnapshotEntity>> = buddies.observeAll()
    suspend fun upsertBuddy(b: com.wkhan.hexis.data.entity.BuddySnapshotEntity) = buddies.upsert(b)
    suspend fun deleteBuddy(id: String) = buddies.deleteById(id)
    val allIntegrityReviews: Flow<List<com.wkhan.hexis.data.entity.IntegrityReviewEntity>> = integrityReviews.observeAll()
    suspend fun upsertIntegrityReview(r: com.wkhan.hexis.data.entity.IntegrityReviewEntity) = integrityReviews.upsert(r)
    suspend fun deleteIntegrityReview(id: String) = integrityReviews.deleteById(id)
    // R35 — third-wave accessors.
    val allExperiments: Flow<List<com.wkhan.hexis.data.entity.ExperimentEntity>> = experiments.observeAll()
    suspend fun upsertExperiment(e: com.wkhan.hexis.data.entity.ExperimentEntity) = experiments.upsert(e)
    suspend fun deleteExperiment(id: String) = experiments.deleteById(id)
    val allActivationItems: Flow<List<com.wkhan.hexis.data.entity.ActivationItemEntity>> = activation.observeAll()
    suspend fun upsertActivationItem(a: com.wkhan.hexis.data.entity.ActivationItemEntity) = activation.upsert(a)
    suspend fun deleteActivationItem(id: String) = activation.deleteById(id)
    val allDayLogs: Flow<List<com.wkhan.hexis.data.entity.DayLogEntity>> = dayLogs.observeAll()
    suspend fun dayLogFor(day: Long): com.wkhan.hexis.data.entity.DayLogEntity? = dayLogs.forDay(day, activeWs())
    // Phase F — the active workspace's day logs (for the evening reminder's skip-if-done + adaptive-time layer).
    suspend fun dayLogsOnce(): List<com.wkhan.hexis.data.entity.DayLogEntity> {
        val ws = activeWs(); return dayLogs.getAll().filter { it.workspaceId == ws }
    }
    suspend fun upsertDayLog(d: com.wkhan.hexis.data.entity.DayLogEntity) = dayLogs.upsert(d)
    // R36 — fourth-wave accessors.
    val allEscrows: Flow<List<com.wkhan.hexis.data.entity.EscrowEntity>> = escrows.observeAll()
    suspend fun upsertEscrow(e: com.wkhan.hexis.data.entity.EscrowEntity) = escrows.upsert(e)
    suspend fun deleteEscrow(id: String) = escrows.deleteById(id)
    val allNudgeEvents: Flow<List<com.wkhan.hexis.data.entity.NudgeEventEntity>> = nudgeEvents.observeAll()
    suspend fun nudgeForHabitDay(habitId: String, day: Long): com.wkhan.hexis.data.entity.NudgeEventEntity? = nudgeEvents.forHabitDay(habitId, day)
    suspend fun openNudgesSince(sinceDay: Long): List<com.wkhan.hexis.data.entity.NudgeEventEntity> = nudgeEvents.openSince(sinceDay)
    suspend fun upsertNudgeEvent(e: com.wkhan.hexis.data.entity.NudgeEventEntity) = nudgeEvents.upsert(e)
    // R38 — dedicated-calendar accessors.
    val allEventCalendars: Flow<List<com.wkhan.hexis.data.entity.EventCalendarEntity>> = eventCalendars.observeAll()
    suspend fun upsertEventCalendar(c: com.wkhan.hexis.data.entity.EventCalendarEntity) = eventCalendars.upsert(c)
    suspend fun deleteEventCalendar(id: String) = eventCalendars.deleteById(id)
    suspend fun eventCalendarsOnce(): List<com.wkhan.hexis.data.entity.EventCalendarEntity> = eventCalendars.getAll()
    ...
    val allTimeActivities: Flow<List<com.wkhan.hexis.data.entity.TimeActivityEntity>> = timeTrack.observeActivities()
    val allTimeEntries: Flow<List<com.wkhan.hexis.data.entity.TimeEntryEntity>> = timeTrack.observeEntries()
    ...
    suspend fun createTimeActivity(name: String, emoji: String?, colorArgb: Long?, goalMinutesPerDay: Int = 0): String {
        ...
        timeTrack.upsertActivity(com.wkhan.hexis.data.entity.TimeActivityEntity(id, name.trim().ifBlank { "Activity" }, emoji, colorArgb, false, order, now(), goalMinutesPerDay, "", activeWs()))
        return id
    }
    suspend fun upsertTimeActivity(a: com.wkhan.hexis.data.entity.TimeActivityEntity) = timeTrack.upsertActivity(a)
    ...
    suspend fun deleteTimeActivity(id: String) {
        ...
        val rules = com.wkhan.hexis.domain.AutomationRules.parse(s.automationRulesJson)
    ...
    suspend fun startTimeTracking(activityId: String, taskId: String? = null, habitId: String? = null, stopFirst: Boolean = true, startMillis: Long? = null, kind: String = "manual", noteId: String? = null): String {
        ...
        timeTrack.upsertEntry(com.wkhan.hexis.data.entity.TimeEntryEntity(id, activityId, start, null, "", taskId, habitId, now(), kind = kind, workspaceId = activeWs(), noteId = noteId))
        return id
    }
    ...
    suspend fun logNoteTime(noteId: String, startMillis: Long, endMillis: Long, note: String = "") {
        ...
        timeTrack.upsertEntry(com.wkhan.hexis.data.entity.TimeEntryEntity(
    ...
    suspend fun habitJournalNote(habitId: String): com.wkhan.hexis.data.entity.NoteEntity? =
    ...
    suspend fun appendHabitJournalEntry(habitId: String, epochDay: Long) {
        ...
        if (note.vault && com.wkhan.hexis.domain.NoteVault.isLocked(note.body)) return
    ...
    private suspend fun finalizeEntry(running: com.wkhan.hexis.data.entity.TimeEntryEntity) {
        val end = now()
    ...
    suspend fun addManualTimeEntry(activityId: String, startMillis: Long, endMillis: Long, note: String = "", taskId: String? = null, habitId: String? = null) =
        timeTrack.upsertEntry(com.wkhan.hexis.data.entity.TimeEntryEntity(uid(), activityId, startMillis, endMillis, note, taskId, habitId, now(), workspaceId = activeWs()))
    suspend fun upsertTimeEntry(e: com.wkhan.hexis.data.entity.TimeEntryEntity) = timeTrack.upsertEntry(e)
    ...
    suspend fun runningTimeEntry(): com.wkhan.hexis.data.entity.TimeEntryEntity? = timeTrack.runningEntry()
    suspend fun getTimeActivitiesOnce(): List<com.wkhan.hexis.data.entity.TimeActivityEntity> = timeTrack.getActivities()
    ...
    suspend fun mirrorFocusInterval(startMillis: Long, endMillis: Long, activityId: String, taskId: String?, habitId: String?) {
        ...
        timeTrack.upsertEntry(
            com.wkhan.hexis.data.entity.TimeEntryEntity(
    ...
    fun observeNotes(): Flow<List<com.wkhan.hexis.data.entity.NoteEntity>> = notes.observeAll()
    /** W2 (scale) — DB-side workspace+trashed filter (index-backed), replacing the in-memory VM filter. */
    fun observeNotesByWorkspace(ws: String, trashed: Boolean): Flow<List<com.wkhan.hexis.data.entity.NoteEntity>> =
    ...
    fun observeNotebooks(): Flow<List<com.wkhan.hexis.data.entity.NotebookEntity>> = notebooks.observeAll()
    // W3 (cross-module unification) — Goals & Routines and their logs live in Room; the settings
    // `goals`/`goal_reviews`/`routines`/`routine_runs` k/v entries are kept only as the backward-compatible
    // BACKUP transport (regenerated from the table at export, consumed at import — that path stays here and
    // reads the `goals`/`routines` DAOs directly). Safe partial decomposition: the whole Goals+Routines data
    // module (self-contained — its own tables only) lives in GoalsRoutinesRepository now; these one-line shims
    // keep every external call site (`repo.observeGoals()`, `repo.deleteRoutine()`, …) unchanged.
    private val goalsRoutines = GoalsRoutinesRepository(db)
    fun observeGoals() = goalsRoutines.observeGoals()
    ...
    suspend fun wsGoalsOnce(): List<com.wkhan.hexis.domain.Goal> {
        val ws = activeWs()
        return goalsFromTableOnce().map { it.toDomain() }
            .filter { it.workspaceId.ifBlank { com.wkhan.hexis.data.entity.WorkspaceEntity.DEFAULT_ID } == ws }
    }
    /** Item 13 — every goal review as a domain object (portfolio reviews are workspace-agnostic). */
    suspend fun goalReviewsOnce(): List<com.wkhan.hexis.domain.GoalReview> =
    ...
    suspend fun routineRunsOnce() = goalsRoutines.routineRunsOnce()
    suspend fun routinesFromTableOnce() = goalsRoutines.routinesFromTableOnce()
    suspend fun routineRunsFromTableOnce() = goalsRoutines.routineRunsFromTableOnce()
    suspend fun replaceWorkspaceRoutines(ws: String, list: List<com.wkhan.hexis.domain.Routine>) = goalsRoutines.replaceWorkspaceRoutines(ws, list)
    suspend fun deleteRoutine(id: String) = goalsRoutines.deleteRoutine(id)
    suspend fun appendRoutineRun(run: com.wkhan.hexis.domain.RoutineRun) = goalsRoutines.appendRoutineRun(run)
    suspend fun reconcileRoutinesFromLegacyJson(routinesJson: String, runsJson: String) = goalsRoutines.reconcileRoutinesFromLegacyJson(routinesJson, runsJson)
    suspend fun replaceWorkspaceGoals(ws: String, list: List<com.wkhan.hexis.domain.Goal>) = goalsRoutines.replaceWorkspaceGoals(ws, list)
    suspend fun deleteGoal(id: String) = goalsRoutines.deleteGoal(id)
    suspend fun replaceGoalReviews(list: List<com.wkhan.hexis.domain.GoalReview>) = goalsRoutines.replaceGoalReviews(list)
    suspend fun reconcileGoalsFromLegacyJson(goalsJson: String, reviewsJson: String) = goalsRoutines.reconcileGoalsFromLegacyJson(goalsJson, reviewsJson)
    suspend fun getNotesOnce(): List<com.wkhan.hexis.data.entity.NoteEntity> = notes.getAll()
    suspend fun getNote(id: String): com.wkhan.hexis.data.entity.NoteEntity? = notes.getById(id)
    ...
    suspend fun getNotebooksOnce(): List<com.wkhan.hexis.data.entity.NotebookEntity> = notebooks.getAll()

    /** Create (or update) a note, stamping timestamps + sort order, and keep the FTS index fresh. */
    suspend fun upsertNote(n: com.wkhan.hexis.data.entity.NoteEntity): String {
        ...
        val vaulted = n.vault && com.wkhan.hexis.domain.NoteVault.isLocked(n.body)
        val stamped = n.copy(
            id = id,
            ...
            preview = if (vaulted) "🔒 Locked" else com.wkhan.hexis.domain.NoteDerived.preview(n.body),
            hasOpen = if (vaulted) false else com.wkhan.hexis.domain.NoteDerived.hasOpenItems(n.body),
        )
    ...
    private suspend fun materializeNoteCards(note: com.wkhan.hexis.data.entity.NoteEntity) {
        val parsed = com.wkhan.hexis.domain.NoteCards.parse(note.id, note.body)
        ...
        for (c in parsed) {
            ...
            if (existing == null) {
                val s = com.wkhan.hexis.domain.NoteCards.fresh(nowMs)
                noteCards.upsert(com.wkhan.hexis.data.entity.NoteCardEntity(
    ...
    suspend fun dueNoteCards(nowMs: Long = now()): List<com.wkhan.hexis.data.entity.NoteCardEntity> = noteCards.due(nowMs)
    suspend fun noteCardsForNote(noteId: String): List<com.wkhan.hexis.data.entity.NoteCardEntity> = noteCards.forNote(noteId)

    /** Apply a grade to a card (SM-2), persist the new schedule, and return the updated row. */
    suspend fun gradeNoteCard(id: String, grade: com.wkhan.hexis.domain.NoteCards.Grade): com.wkhan.hexis.data.entity.NoteCardEntity? {
        val card = noteCards.byId(id) ?: return null
        val nowMs = now()
        val s = com.wkhan.hexis.domain.NoteCards.schedule(
            com.wkhan.hexis.domain.NoteCards.Sched(card.easiness, card.intervalDays, card.reps, card.lapses, card.dueAt),
            grade, nowMs)
    ...
    suspend fun goalJournalNote(goalId: String): com.wkhan.hexis.data.entity.NoteEntity? =
    ...
    suspend fun syncBoundCheckboxTasks(noteId: String) {
        ...
        val boxes = com.wkhan.hexis.domain.NoteCheckboxSync.boundBoxes(n.body)
    ...
    suspend fun reconcileNoteCheckboxesFromTasks(noteId: String) {
        val n = notes.getById(noteId) ?: return
        if (!com.wkhan.hexis.domain.NoteCheckboxSync.hasBound(n.body)) return
        val byTitle = tasks.getAll().filter { !it.trashed }.associateBy { it.title.trim().lowercase() }
        val newBody = com.wkhan.hexis.domain.NoteCheckboxSync.reconcile(n.body) { title -> byTitle[title.trim().lowercase()]?.completed }
    ...
    suspend fun materializeNoteTags(noteId: String) {
        val n = notes.getById(noteId) ?: return
        val names = com.wkhan.hexis.domain.NoteGrammar.TAG.findAll(n.body)
        ...
        val refs = names.map { name ->
            val id = byName[name.lowercase()]?.id ?: uid().also {
                tags.upsert(com.wkhan.hexis.data.entity.TagEntity(it, name, workspaceId = ws))
            }
            com.wkhan.hexis.data.entity.NoteTagCrossRef(noteId, id)
        }
        notes.linkTags(refs)
    }
    ...
    suspend fun reindexNoteFts(note: com.wkhan.hexis.data.entity.NoteEntity) {
        val extra = buildList {
    ...
    suspend fun materializeNoteContexts(noteId: String) {
        val n = notes.getById(noteId) ?: return
        val names = com.wkhan.hexis.domain.NoteGrammar.CONTEXT.findAll(n.body)
            .map { it.groupValues[1] }.filter { it.isNotBlank() }.distinctBy { it.lowercase() }.toList()
        if (names.isEmpty()) return
        val ws = n.workspaceId
        ...
        val refs = names.map { name ->
            val id = byName[name.lowercase()]?.id ?: uid().also {
                contexts.upsert(com.wkhan.hexis.data.entity.ContextEntity(id = it, name = name, workspaceId = ws))
            }
            com.wkhan.hexis.data.entity.NoteContextCrossRef(noteId, id)
        }
     ...
     * SQL foreign keys (see [com.wkhan.hexis.data.entity.NoteEntity]'s KDoc for why), so the
    ...
    suspend fun setNoteTags(noteId: String, tagIds: List<String>) {
        ...
        notes.linkTags(tagIds.map { com.wkhan.hexis.data.entity.NoteTagCrossRef(noteId, it) })
    ...
    suspend fun setNoteContexts(noteId: String, contextIds: List<String>) {
        ...
        notes.linkContexts(contextIds.map { com.wkhan.hexis.data.entity.NoteContextCrossRef(noteId, it) })
        notes.getById(noteId)?.let { reindexNoteFts(it) }   // L2 — keep context names in the FTS index fresh
    }
    suspend fun getNoteTagCrossRefs(): List<com.wkhan.hexis.data.entity.NoteTagCrossRef> = notes.getTagCrossRefs()
    /** Live note↔tag links — so the editor reflects a tag toggle immediately (writing note_tags doesn't touch the notes table). */
    fun observeNoteTagCrossRefs(): kotlinx.coroutines.flow.Flow<List<com.wkhan.hexis.data.entity.NoteTagCrossRef>> = notes.observeTagCrossRefs()
    /** Live note↔context links — so the editor reflects a context toggle immediately (mirrors tags). */
    fun observeNoteContextCrossRefs(): kotlinx.coroutines.flow.Flow<List<com.wkhan.hexis.data.entity.NoteContextCrossRef>> = notes.observeContextCrossRefs()
    // Wave L — a note's attachment rows (for the rich renderer's inline-image resolution).
    suspend fun noteAttachments(noteId: String): List<com.wkhan.hexis.data.entity.AttachmentEntity> = notes.attachmentsForNote(noteId)
    suspend fun getNoteContextCrossRefs(): List<com.wkhan.hexis.data.entity.NoteContextCrossRef> = notes.getContextCrossRefs()

    suspend fun upsertNotebook(nb: com.wkhan.hexis.data.entity.NotebookEntity): String {
    ...
    suspend fun duplicateNote(id: String): String? {
        ...
        if (tagIds.isNotEmpty()) notes.linkTags(tagIds.map { com.wkhan.hexis.data.entity.NoteTagCrossRef(newId, it) })
        if (ctxIds.isNotEmpty()) notes.linkContexts(ctxIds.map { com.wkhan.hexis.data.entity.NoteContextCrossRef(newId, it) })
        notes.getById(newId)?.let { syncNoteFts(newId, it.title, it.body) }
        return newId
    }

    fun observeNoteRevisions(noteId: String): Flow<List<com.wkhan.hexis.data.entity.NoteRevisionEntity>> = noteRevisions.observeForNote(noteId)
    suspend fun getNoteRevisionsOnce(): List<com.wkhan.hexis.data.entity.NoteRevisionEntity> = noteRevisions.getAll()
    ...
    suspend fun saveNoteRevision(noteId: String, keep: Int) {
        ...
        noteRevisions.insert(com.wkhan.hexis.data.entity.NoteRevisionEntity(
    ...
    suspend fun materializeNoteLinks(noteId: String) {
        ...
        val titles = com.wkhan.hexis.domain.NoteLinks.outgoingTitles(n.body)
        ...
        val rows = titles.map { t ->
            ...
            when {
                note != null -> com.wkhan.hexis.data.entity.NoteLinkEntity(noteId, t, "note", note.id)
                task != null -> com.wkhan.hexis.data.entity.NoteLinkEntity(noteId, t, "task", task.id)
                habit != null -> com.wkhan.hexis.data.entity.NoteLinkEntity(noteId, t, "habit", habit.id)
                event != null -> com.wkhan.hexis.data.entity.NoteLinkEntity(noteId, t, "event", event.id)
                else -> com.wkhan.hexis.data.entity.NoteLinkEntity(noteId, t, "note", "")
            }
    ...
    fun observeNoteLinks(noteId: String): kotlinx.coroutines.flow.Flow<List<com.wkhan.hexis.data.entity.NoteLinkEntity>> = noteLinks.observeForNote(noteId)
    suspend fun getNoteLinksOnce(): List<com.wkhan.hexis.data.entity.NoteLinkEntity> = noteLinks.getAll()
    ...
    suspend fun unlinkedMentions(body: String, excludeNoteId: String): List<String> {
        ...
        val linked = com.wkhan.hexis.domain.NoteLinks.outgoingTitles(body).map { it.lowercase() }.toSet()
     ...
     *  this; Hexis can, because the day's real data lives right here. */
    ...
    // ---- Wave D: Smart Views ----
    fun observeSmartViews(): kotlinx.coroutines.flow.Flow<List<com.wkhan.hexis.data.entity.SmartViewEntity>> = smartViews.observeAll()
    suspend fun upsertSmartView(v: com.wkhan.hexis.data.entity.SmartViewEntity): String {
    ...
    suspend fun quickCaptureTask(text: String): String? {
        ...
        val parsed = com.wkhan.hexis.domain.nlp.QuickAddParser.parse(tok.text)
        ...
        if (parsed.hasTime && due != null)
            upsertReminder(com.wkhan.hexis.data.entity.ReminderEntity(uid(), taskId = id, type = "absolute", atTime = due))
        return id
    }
    ...
    private suspend fun hydratedAttachments(): List<AttachmentEntity> = attachments.getAll().map { a ->
        if (a.contentBase64.isBlank() && !a.filePath.isNullOrBlank()) {
            ...
            if (f.exists()) a.copy(contentBase64 = android.util.Base64.encodeToString(com.wkhan.hexis.data.security.FileVault.readDecrypted(f), android.util.Base64.NO_WRAP), filePath = null) else a
        } else a
    }
    ...
    suspend fun wsCountdownsOnce(): List<com.wkhan.hexis.data.entity.CountdownEntity> =
    ...
    suspend fun wsTimeActivitiesOnce(): List<com.wkhan.hexis.data.entity.TimeActivityEntity> =
        timeTrack.getActivities().filter { it.workspaceId == activeWs() }
    suspend fun wsFocusSessionsOnce(): List<com.wkhan.hexis.data.entity.FocusSessionEntity> =
        focus.getAll().filter { it.workspaceId == activeWs() }
    suspend fun wsActivitiesOnce(): List<com.wkhan.hexis.data.entity.ActivityEntity> {
        val taskIds = wsTasksOnce().map { it.id }.toSet()
        return activity.getAll().filter { it.taskId in taskIds }
    }
    suspend fun wsEventsOnce(): List<com.wkhan.hexis.data.entity.EventEntity> {
    ...
    private suspend fun exportableSettings(): List<SettingEntity> {
        val K = com.wkhan.hexis.domain.AppSettings.Keys
        ...
        val goalsJson = com.wkhan.hexis.domain.Goals.encode(goals.getAll().map { it.toDomain() })
        val reviewsJson = com.wkhan.hexis.domain.GoalReviews.encode(goals.getAllReviews().map { it.toDomain() })
        ...
        val routinesJson = com.wkhan.hexis.domain.Routines.encode(routines.getAll().map { it.toDomain() })
        val runsJson = com.wkhan.hexis.domain.RoutineRuns.encode(routines.getAllRuns().map { it.toDomain() })
    ...
    suspend fun exportMarkdown(includeCompleted: Boolean): String =
        com.wkhan.hexis.domain.port.Export.toMarkdown(
    ...
    suspend fun exportIcs(includeCompleted: Boolean): String =
        com.wkhan.hexis.domain.port.Export.toIcs(
    ...
    suspend fun exportHabitsCsv(): String =
        com.wkhan.hexis.domain.port.Export.toHabitsCsv(habits.getAll(), habits.getCheckins())
    ...
    suspend fun importHabitsCsv(text: String): Int {
        val res = com.wkhan.hexis.data.sync.HabitImporter.parse(text) ?: return 0
        ...
        res.rows.groupBy { it.habit }.forEach { (name, rows) ->
            val h = existing[name.lowercase()] ?: run {
                val id = createHabit(name.trim(), null, null, 1, com.wkhan.hexis.data.entity.WorkspaceEntity.DEFAULT_ID)
    ...
    suspend fun exportCsv(includeCompleted: Boolean): String =
        com.wkhan.hexis.domain.port.Export.toCsv(
            tasks = tasks.getAll(), lists = lists.getAll(), tags = tags.getAll(),
            taskTagPairs = tags.getCrossRefs().map { it.taskId to it.tagId }, includeCompleted = includeCompleted,
    ...
    suspend fun importJsonReplace(text: String) {
        ...
        val keepPass = settings.getAll().firstOrNull { it.key == com.wkhan.hexis.domain.AppSettings.Keys.SYNC_PASS }
        ...
        if (keepPass != null && b.settings.none { it.key == com.wkhan.hexis.domain.AppSettings.Keys.SYNC_PASS }) settings.putAll(listOf(keepPass))
        ...
        run {
            ...
            goals.upsertAll(com.wkhan.hexis.domain.Goals.parse(gj).map { it.toEntity() })
            goals.upsertReviews(com.wkhan.hexis.domain.GoalReviews.parse(rj).map { it.toEntity() })
            ...
            routines.upsertAll(com.wkhan.hexis.domain.Routines.parse(roj).map { it.toEntity() })
            routines.upsertRuns(com.wkhan.hexis.domain.RoutineRuns.parse(ruj).map { it.toEntity() })
        }
    ...
    suspend fun importJsonMerge(text: String) {
        ...
        val merged = com.wkhan.hexis.data.sync.SyncEngine.merge(snapshot(), incoming)
        applyMerged(merged)
    ...
    private suspend fun mergeNewerTables(b: com.wkhan.hexis.domain.port.BackupFile, includeAttachments: Boolean = true) {
    ...
    suspend fun snapshot(): com.wkhan.hexis.domain.port.BackupFile =
        com.wkhan.hexis.domain.port.BackupFile(
            exportedAt = now(),
    ...
    suspend fun applyMerged(b: com.wkhan.hexis.domain.port.BackupFile) {
