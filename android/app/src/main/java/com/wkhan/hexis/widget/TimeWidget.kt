...
class TimeWidget : BaseWidgetProvider() {
    ...
    companion object {
        fun refresh(context: Context) = Widgets.broadcastUpdate(context, TimeWidget::class.java)
    }
}
            ...
            try {
                when (intent.action) {
                    ACTION_START -> intent.getStringExtra(EXTRA_ACTIVITY_ID)?.let { actId ->
                        ...
                        com.wkhan.hexis.reminders.AutomationRunner.onStart(context, app.repository, actId)
                    }
                    ACTION_STOP -> {
...
const val ACTION_START = "com.wkhan.hexis.action.TIME_START"
const val ACTION_STOP = "com.wkhan.hexis.action.TIME_STOP"
