package com.wkhan.hexis.data.backup
...
import com.wkhan.hexis.data.sync.Crypto
import com.wkhan.hexis.data.sync.Importers
import com.wkhan.hexis.data.sync.SyncEngine
import com.wkhan.hexis.domain.AppSettings
import com.wkhan.hexis.reminders.AlarmScheduler
import com.wkhan.hexis.util.FileExport
...
class RestoreManager(
    ...
    suspend fun importPastedText(text: String, onDone: (Boolean, String) -> Unit) {
        ...
        if (t.startsWith("{") || t.startsWith("[")) {
            ...
            onDone(ok, if (ok) "Restored from pasted backup" else "That text isn't a valid Hexis backup")
    ...
    suspend fun restoreSaved(s: FileExport.SavedFile, onDone: (Boolean, String) -> Unit) {
        ...
        if (s.name.endsWith(".json", ignoreCase = true)) {
            ...
            onDone(ok, if (ok) "Restored from ${s.name}" else "Restore failed — is this a Hexis backup?")
    ...
    suspend fun importFromIntent(uri: Uri, merge: Boolean = false, onDone: (Boolean, String) -> Unit) {
        ...
        val text = readImportText(uri = uri)
        if (text.isNullOrBlank()) { onDone(false, "Couldn't read that file. Try 'Share → Hexis' from your file manager."); return }
        ...
        when {
            ...
            looksJson -> {
                ...
                else onDone(false, "That file isn't a valid Hexis backup")
            }
            else -> importExternalText(text, onDone)
        }
    }
