...
import com.wkhan.hexis.ui.QuickAddOptions
import com.wkhan.hexis.ui.components.DateReminderSheet
...
private fun QuickAddBody(vm: AppViewModel, initialDue: Long? = null, initialHasTime: Boolean = false, initialText: String = "", onDismiss: () -> Unit) {
    ...
    val capTok = remember(text) { com.wkhan.hexis.domain.nlp.QuickTokens.parse(text, handleActivity = false) }
    val capParsed = remember(capTok.text) { com.wkhan.hexis.domain.nlp.QuickAddParser.parse(capTok.text) }
    ...
    Column(Modifier.fillMaxWidth().imePadding().padding(horizontal = 20.dp, vertical = 6.dp)) {
        ...
        val destText = when {
            ...
            destView is com.wkhan.hexis.domain.view.ViewRef.FolderView ->
                "📁 " + (folders.firstOrNull { it.id == (destView as com.wkhan.hexis.domain.view.ViewRef.FolderView).folderId }?.name ?: "Folder")
            destView is com.wkhan.hexis.domain.view.ViewRef.ListView ->
                lists.firstOrNull { it.id == (destView as com.wkhan.hexis.domain.view.ViewRef.ListView).listId }?.name ?: "List"
            else -> "Inbox"
        }
                ...
                Box {
                    ...
                    DropdownMenu(expanded = moreMenu, onDismissRequest = { moreMenu = false }) {
                        DropdownMenuItem(
                            ...
                            onClick = {
                                moreMenu = false
                                com.wkhan.hexis.util.SystemPicker.openFiles(arrayOf("*/*"), onError = { android.widget.Toast.makeText(qaCtx, it, android.widget.Toast.LENGTH_LONG).show() }) { uris -> attachments = attachments + uris }
                            })
                        DropdownMenuItem(
    ...
    if (showPrio) com.wkhan.hexis.ui.components.PrioritySheet(
...
// Theme-aware: reads the shared HexisColors chart/semantic tokens (light + dark sets) instead of fixed
// literals, so the capture chips adapt to dark/AMOLED like the live highlighter already does.
@androidx.compose.runtime.Composable
private fun captureChipColor(t: com.wkhan.hexis.domain.nlp.ChipType): Color {
    val k = com.wkhan.hexis.ui.theme.LocalHexisColors.current
    return when (t) {
        com.wkhan.hexis.domain.nlp.ChipType.DATE, com.wkhan.hexis.domain.nlp.ChipType.TIME -> k.info
        com.wkhan.hexis.domain.nlp.ChipType.PRIORITY -> k.chart[2]   // amber
        com.wkhan.hexis.domain.nlp.ChipType.TAG -> k.chart[4]        // violet
        com.wkhan.hexis.domain.nlp.ChipType.CONTEXT -> k.chart[7]    // pink
        com.wkhan.hexis.domain.nlp.ChipType.LIST -> k.chart[1]       // teal
        com.wkhan.hexis.domain.nlp.ChipType.REMINDER -> k.chart[5]   // sky
        com.wkhan.hexis.domain.nlp.ChipType.RECUR -> k.chart[0]      // indigo (brand)
        com.wkhan.hexis.domain.nlp.ChipType.ESTIMATE -> k.chart[5]   // sky
        com.wkhan.hexis.domain.nlp.ChipType.STAR -> k.star
    }
}
