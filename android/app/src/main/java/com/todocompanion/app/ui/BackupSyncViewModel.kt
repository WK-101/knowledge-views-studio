...
class BackupSyncViewModel(
    ...
    private val backup by lazy { com.todocompanion.app.data.backup.BackupExporter(app.appCtx, repo) { zone } }
    ...
    private fun rearmAutoBackup(s: com.todocompanion.app.domain.AppSettings) {
        if (s.autoBackupEnabled && s.autoBackupFolder.ifBlank { s.syncFolder }.isNotBlank())
            com.todocompanion.app.reminders.AlarmScheduler.scheduleAutoBackup(app.appCtx, s.autoBackupHour, s.autoBackupIntervalDays, s.lastBackupAt, s.autoBackupDow, s.autoBackupDom)
        else com.todocompanion.app.reminders.AlarmScheduler.cancelAutoBackup(app.appCtx)
    }
    ...
    fun loadSavedBackups(broad: Boolean = false, onDone: (List<com.todocompanion.app.util.FileExport.SavedFile>) -> Unit) = scope.launch { onDone(restore.loadSavedBackups(broad)) }
    ...
    fun restoreSaved(s: com.todocompanion.app.util.FileExport.SavedFile, onDone: (Boolean, String) -> Unit) = scope.launch { restore.restoreSaved(s, onDone) }
