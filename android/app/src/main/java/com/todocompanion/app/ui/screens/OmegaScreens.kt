...
fun CommandPaletteDialog(vm: AppViewModel, onDismiss: () -> Unit, onRun: (OmegaCommand.Command) -> Unit) {
    ...
    Dialog(onDismissRequest = onDismiss) {
        Surface(shape = RoundedCornerShape(20.dp), color = MaterialTheme.colorScheme.surface, tonalElevation = 4.dp) {
            Column(Modifier.fillMaxWidth().padding(18.dp)) {
                ...
                if (text.isBlank()) {
                    ...
                    val examples = listOf(
                        "track deep work", "go to habits", "plan my day",
                        "hours on Reading this week", "tasks done last week", "best habit", "year in review",
                    )
                    FlowRowCompat {
                        examples.forEach { ex ->
