 ...
 *   TCENC3:<base64( salt[16] || iv[12] || ciphertext+tag )>   ← current, PBKDF2 600k iterations
 *   TCENC2:<base64( salt[16] || iv[12] || ciphertext+tag )>   ← legacy, 210k iterations (still read)
 ...
 * decrypts under the count it was written with. New files are always written as TCENC3, raising the
 * PBKDF2 work factor to OWASP's current (2023) PBKDF2-HMAC-SHA256 guidance of 600k. A previously-
 * exported file is transparently upgraded to the current marker the next time the app re-writes it
 * (no user action, no re-key). PBKDF2 is not memory-hard; the marker scheme is exactly the seam a
 * future memory-hard KDF (Argon2id) would slot into as a new marker without disturbing old files.
 */
object Crypto {
    private const val MARKER_V3 = "TCENC3:"
    private const val MARKER_V2 = "TCENC2:"   // legacy, decrypt-only
    private const val MARKER_V1 = "TCENC1:"   // legacy, decrypt-only
    private const val ITER_V3 = 600_000
    ...
    fun isEncrypted(text: String): Boolean =
        text.startsWith(MARKER_V3) || text.startsWith(MARKER_V2) || text.startsWith(MARKER_V1)
    ...
    fun encrypt(plaintext: String, passphrase: String): String {
        ...
        val key = deriveKey(passphrase.toCharArray(), salt, ITER_V3)
        val cipher = Cipher.getInstance("AES/GCM/NoPadding")
        cipher.init(Cipher.ENCRYPT_MODE, key, GCMParameterSpec(TAG_BITS, iv))
        val ct = cipher.doFinal(plaintext.toByteArray(Charsets.UTF_8))
        val blob = salt + iv + ct
        return MARKER_V3 + Base64.encodeToString(blob, Base64.NO_WRAP)
    }
     ...
     * Reads v3 (600k), v2 (210k) and legacy v1 (120k) blobs, picking the iteration count by marker.
     */
    fun decrypt(text: String, passphrase: String): String? {
        val iterations: Int
        val marker: String
        when {
            text.startsWith(MARKER_V3) -> { iterations = ITER_V3; marker = MARKER_V3 }
